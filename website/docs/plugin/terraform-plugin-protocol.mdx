...
| `PrepareProviderConfig`      | `ValidateProviderConfig`     | Validates the practitioner supplied provider configuration by verifying types conform to the schema and allows for custom or advanced validation.                                                                                    |
| `ValidateResourceTypeConfig` | `ValidateResourceConfig`     | Validates the practitioner supplied resource configuration by verifying types conform to the schema and allows for custom or advanced validation.                                                                                    |
| `ValidateDataSourceConfig`   | `ValidateDataResourceConfig` | Validates the practitioner supplied data source configuration by verifying types conform to the schema and allows for custom or advanced validation.                                                                                 |
| `UpgradeResourceState`       | `UpgradeResourceState`       | Called when a resource in state has a schema that doesn't match the current version. The provider is expected to modify the state to upgrade it to the latest schema.                                                                |
| `Configure`                  | `ConfigureProvider`          | Passes the practitioner-specified provider configuration to the provider.                                                                                                                                                            |
...
| `ApplyResourceChange`        | `ApplyResourceChange`        | Called when a diff between the resource's state and configuration has been detected, and the practitioner has approved a planned change. The provider is to apply the changes contained in the plan, and return the resulting state. |
