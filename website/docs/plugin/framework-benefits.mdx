...
- **[Terraform Plugin Framework](/plugin/framework):** The most recent SDK that is easier to use and more extensible than SDKv2.
- **[SDKv2](/plugin/sdkv2):** SDKv2 is the prior SDK that many existing providers use. It is maintained for Terraform versions 1.x and earlier, but we have stopped most feature development so we can focus on improving the framework.

We recommend using the framework for new provider development because it offers significant advantages as compared to the SDKv2. We also recommend migrating existing providers to the framework when possible. If you manage a large existing provider, you can use [`terraform-plugin-mux`](/plugin/mux) to migrate individual resources or data sources to the framework one at a time. Refer to [Migrating from SDK](/plugin/framework/migrating/providers#muxing) for details.

This page is intended for developers experienced with SDKv2 plugins, and explains how the framework makes provider development and maintenance easier. If you are just getting started with provider development, we recommend viewing the [Framework documentation](/plugin/framework) instead. For a continued list of new or improved functionality in the framework, refer to [Framework Feature Comparison](/plugin/framework/migrating/benefits).
...
The framework exposes the sources for data separately and lets you inspect whether a value is null or unknown. Refer to the [Framework Feature Comparison](/plugin/framework/migrating/benefits) for more details and examples.
...
Refer to [Framework Feature Comparison](/plugin/framework/migrating/benefits) for a continued list of features, details, and examples.
