...
A Terraform resource and associated schema should follow the naming and structure of the API, unless it degrades the user experience or works in a way counter to a user's expectations of Terraform. Simplification of an API in Terraform should be accomplished via [Terraform Modules](/language/modules/develop), potentially hosted in the [Terraform Registry](https://registry.terraform.io/). Additional guidance can be found in the [resource and attribute naming guide](/plugin/sdkv2/best-practices/naming).
...
- Terraform does not support recursive types (but this can in some cases be modeled using a dynamic type)
