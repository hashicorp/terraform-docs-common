...
A Terraform resource and associated schema should follow the naming and structure of the API, unless it degrades the user experience or works in a way counter to a user's expectations of Terraform. You can simplify an API in Terraform using [Terraform Modules](/language/modules/develop), potentially hosted in the [Terraform Registry](https://registry.terraform.io/). Refer to [resource and attribute naming guide](/plugin/sdkv2/best-practices/naming) for more information.
...
- Terraform does not support recursive types (but sometimes you can model them using a dynamic type)
- Boolean attributes should indicate that `true` means to do something and `false` means not to do it, sometimes using boolean attributes means inverting the API
