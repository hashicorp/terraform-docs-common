...
1. Copy the [GoReleaser configuration from the terraform-provider-scaffolding-framework repository](https://github.com/hashicorp/terraform-provider-scaffolding-framework/blob/main/.goreleaser.yml) to the root of your repository.
  * *Optionally, if you are interested in bundling your repository's license file with the release artifacts, modify `.gorelease.yml` so that the first array element of the `archives` key looks like this*:
    ```yaml
    archives:
      - files:
          - src: 'LICENSE' # Replace with the license filepath in your project repo if different
            dst: 'LICENSE.txt'
        format: zip
