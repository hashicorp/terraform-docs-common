...
> **Hands-on:** Try the [Call APIs with Terraform Providers](/terraform/tutorials/providers?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorials to create a custom Terraform provider, complete with authentication and create/read/update/delete/import functionality.
...
~> **Important:** If developing a provider using [Terraform Plugin Framework](/terraform/plugin/framework), ensure the protocol versions are set to `["6.0"]` or practitioners will receive errors when attempting to use the provider.
