...
The [Terraform Registry][terraform-registry] displays documentation for the providers it hosts. This page describes the expected format for provider documentation. Provider documentation is _not_ supported in the private registry of [Terraform Cloud or Terraform Enterprise](/terraform/cloud-docs/registry/publish-providers#release-files).
...
The maximum number of documents allowed for a single provider version is 2500.
...
A provider named `example` with a resource and data source for `instance` and a function named `parse_instance_id`  would have these 4 files:
...
1. Expand the folder names to match the Terraform Registry's expected format:
   ...
   * No changes necessary for `docs/functions/` or `docs/guides`
