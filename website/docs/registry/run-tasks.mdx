...
Each run task in a [workspace](/terraform/cloud-docs/workspaces) sends information to an external service at a specific run stage. The external service processes the information and returns a passed or failed status to Terraform Cloud. For example, you could add a run task that analyzes a plan's output before Terraform applies that plan.

Depending on the workspace’s configuration, failed run tasks can stop runs and prevent Terraform from provisioning infrastructure. For example, the [HCP Packer validation run task](/hcp/docs/packer/manage-image-use/terraform-cloud-run-tasks) checks the image artifacts within a Terraform configuration. The run task fails if the configuration references images marked as unusable.
