...
We make every effort to support GitHub Markdown syntax, but there are some limitations. To ensure consistent rendering in the Public Registry, we recommend using generic Markdown syntax whenever possible.

### Why are referenced links not rendered?

Reference link definitions must not have leading spaces. Ensure that all link definitions are properly formatted without any leading spaces to enable proper rendering.
