...
The Terraform Registry at [registry.terraform.io](https://registry.terraform.io) hosts public Terraform resources, but many organizations use modules, providers, or policies that cannot or do not need to be publicly available.

You can load private modules [directly from version control and other sources](/language/modules/sources), but those methods do not support [version constraints](/language/modules/syntax#version) or a browsable marketplace, both of which are important for enabling a producers-and-consumers content model in a large organization. You will benefit from a private registry if your teams need access to a common list of Terraform resources.
