...
Policy libraries are built and maintained on GitHub. Once a policy library has been published, you can release a new version of the policies by pushing a properly formatted Git tag, as described below.
...
- **sentinel.hcl file**: The GitHub repository must include a `sentinel.hcl` file at its root level. This file outlines the names and file locations for policies and policy modules that are included in the policy library. Any policy files in the repository that are not listed in this file will be ignored.

    This is a [Sentinel configuration file](https://docs.hashicorp.com/sentinel/configuration), and it uses the same syntax that a Sentinel author is already familiar with. There are two caveats:
    ...
    2. The `enforcement_level` for a policy does not need to be included and is ignored by the Registry.
...
You may optionally associate your policy library with a set of [Terraform providers](/registry/providers). You may choose up to 10 publicly-available providers, in order to indicate to users which types of Terraform resources the policies in your library are applicable to.

### Best-practices

**Group your policies into libraries by category.** Users looking for useful policies in the Registry often have a particular category in mind, like networking rules, or storage security. Try to design your policy libraries so that they target a single, specific category.

**Publish new versions, don't edit old versions.** If you have already published a version of your policy library and recognize a mistake or something you would like to change, create a new version of the library. If you instead edit the files in the old version of the library, any users who have been depending on those policies will not be able to use them until they review your changes and update their Sentinel configuration files.
