...
[Terraform Cloud Agents](/terraform/cloud-docs/agents) can be utilized to access private VCS repositories through the [request forwarding](/terraform/cloud-docs/agents/request-forwarding) feature. If an agent pool shares both workspace workloads and request forwarding, a malicious actor in a connected workspace could exfiltrate the agent’s API token to access private resources that could contain VCS or other sensitive requests, which may include sensitive data like valid session tokens. We recommend that agent pools that handle request forwarding are not assigned to handle workspace workloads.
