...
Terraform Cloud uses static IP ranges for certain features such as notifications and VCS connections. These IP ranges are retrievable through the [IP Ranges API](/terraform/cloud-docs/api-docs/ip-ranges).

-> **Note:** The IP ranges for each feature returned by the IP Ranges API may overlap. Additionally, these published ranges do not currently allow for execution of Terraform runs against local resources, for e.g. CLI-driven runs.
