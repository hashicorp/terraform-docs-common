...
encrypted using 128 bit AES in CTR mode. The key material itself is round-tripped
...
and a 96-bit nonce), and stored alongside the object, in a pattern known as
_envelope encryption_.

Encryption key material is generated via the Vault transit secret engine's
[datakey generation](https://www.vaultproject.io/api-docs/secret/transit#generate-data-key),
using bit material from the kernel's cryptographically secure pseudo-random
number generator (CSPRNG) as the `context` value. Because blob storage encryption
generates a unique key for each object, and relies on envelope encryption,
the encryption key material itself for any given object is not rotated.
