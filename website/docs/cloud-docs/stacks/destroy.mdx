...
This guide explains how to destroy a Stack in HCP Terraform. We recommend destroying a Stack in phases to ensure HCP Terraform destroys your infrastructure safely. The order we recommend is to start by destroying your Stack deployment’s infrastructure, then the deployments themselves, and then the Stack itself.
