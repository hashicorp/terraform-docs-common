...
When running Sentinel policies as policy evaluations (if you opt into the `enhanced` workflow), both `soft-mandatory` and `hard-mandatory` enforcement levels are internally converted to `mandatory` enforcement level, similar to OPA.
