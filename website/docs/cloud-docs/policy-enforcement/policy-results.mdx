...
When Sentinel policies are run as policy evaluations (if opting in to the `enhanced` workflow), then internally both `soft-mandatory` as well as `hard-mandatory` enforcement_levels are converted to `mandatory` enforcement level (similar to OPA).
