...
- Sentinel policy evaluations occur after Terraform completes the plan and after both [run tasks](/cloud-docs/workspaces/settings/run-tasks) and [cost estimation](/cloud-dodcs/cost-estimation). This lets you write Sentinel policies to restrict costs based on the data in the cost estimates.
