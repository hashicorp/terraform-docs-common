...
Policy checks use the latest version of the Sentinel runtime and have access to cost estimation data.

### Policy Evaluations

Policy evaluations are run within the [Terraform Cloud Agent](/terraform/cloud-docs/agents) in Terraform Cloud's infrastructure.
Policy evaluations do not have access to cost estimation data.
