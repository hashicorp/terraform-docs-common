...
Terraform Cloud will execute the policy set using the specified version and the latest plan data for the selected workspace. It will display the evaluation results. If the evaluation has an `Errored` status, inspect the JSON output to determine whether the issue is related to a non-compliant resource or is due to a syntax issue within a policy.
