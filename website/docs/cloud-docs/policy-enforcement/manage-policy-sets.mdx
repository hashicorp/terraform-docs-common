...
description: >-
 Group individual policies into policy sets and apply those policy sets to workspaces. For each run, Terraform Cloud checks the Terraform plan against the policy set.
---
