...
1. **(Sentinel Only) Choose a policy set type:
   ...
   - **Enhanced:** After opting in to the 'enhanced' workflow, you can select the Sentinel runtime version against which the policies in the policy set will be evaluated. This allows Sentinel policies to be evaluated in the [Terraform Cloud Agent](/terraform/cloud-docs/agents).
...
## Pinning policy tool versions to policy sets

-> **Note:** Policy runtime version management is currently in beta.

You can pin a policy set to a specific version of the tool. This provides control over the versioning of the policy-as-code runtime.

#### OPA Policy sets

For OPA Policy sets, a `Runtime version` drop-down selector is available that allows you to choose supported versions of OPA. All policies within this policy set will be evaluated against the chosen policy tool version.
...
All policy sets that are configured to use the `enhanced` workflows will be evaluated in the [Terraform Cloud agent](/terraform/cloud-docs/agents).

-> **Note:** Policy sets that use the `enhanced` workflow do not have access to cost estimation data.
...
-> **Note:** Sentinel tool version pinning is supported only for Sentinel 0.23.0 and above, as well as Terraform Cloud agent versions 1.14.0 and above
