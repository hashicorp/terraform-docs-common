...
## Comparing policy checks and evaluations

Sentinel policy sets can run as both policy checks and evaluations, while OPA policy sets only run as evaluations. Policy checks and evaluations can access different types of data and enable slightly different workflows.
...
Terraform Cloud will execute the policy set using the specified version and the latest plan data for the selected workspace. It will display the evaluation results. If the evaluation fails, FILL HERE.
