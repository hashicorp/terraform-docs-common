...
For Sentinel Policy sets, policy tool version pinning is available on an opt-in basis. Sentinel policy sets will default to using the policy check flow and you will be able to opt-in to the policy evaluation flow on a per-policy set basis through a radio button living on the Sentinel policy set configuration form.
After opting in to the `enhanced` workflow, you can choose the Sentinel runtime version (via the a `Runtime version` drop-down selector) against which the policies in that particular policy set will be evaluated.
Opting in also means that the policy sets are now evaluated in the [Terraform Cloud agent](/terraform/cloud-docs/agents). These policy sets will also not have access to the cost estimation data.
...
-> **Note:** Only Sentinel tool version of 0.23.0 and above and Terraform Cloud agent version 1.14.0 and above support Sentinel tool version pinning
