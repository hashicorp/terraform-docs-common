...
Though policy checks and policy evaluations seem like synonyms, there is a fundamental difference between the policy check and policy evaluation workflow.

### Policy Checks

Sentinel policies are currently run as a policy check. This is the default workflow for Sentinel.
Policy checks are pinned to a single runtime version of Sentinel and have access to cost estimation data.

### Policy Evaluations

Policy evaluations are run within the [Terraform Cloud Agent](/terraform/cloud-docs/agents).
OPA policies run as policy evaluations. Policy evalutations do not have access to cost estimation data.

-> **Note:** Sentinel policy evaluations is currently in beta.
...
1. **(Sentinel Only)** Choose a policy set type:
   - **Standard:** This is the default workflow. Sentinel policy set is run as a policy check in Terraform Cloud.
   - **Enhanced:** Sentinel policy set is run as a policy evaluation in Terraform Cloud.
...
For OPA Policy sets, a `Runtime version` drop-down selector is available that allows you to choose supported versions of OPA. All policies within this policy set will be evaluated against the chosen policy runtime version.

#### Sentinel Policy sets

For Sentinel Policy sets, policy runtime version selection is available on an opt-in basis. Sentinel policy sets will default to using the policy check flow, and you can opt in to the policy evaluation flow for each policy set individually through a radio button located in the Sentinel policy set configuration form.

After opting in to the `enhanced` workflow, you can select the Sentinel runtime version (via the `Runtime version` drop-down selector) against which the policies in the policy set will be evaluated.

All policy sets that are configured to use the `enhanced` workflows will run as a policy evaluation.

The Sentinel Policy sets will also have an `override` checkbox (similar to OPA). Failed policies stop the run, but any user with [Manage Policy Overrides permission](/terraform/cloud-docs/users-teams-organizations/permissions#manage-policy-overrides) can override these failures and allow the run to complete.
Both soft-mandatory and hard-mandatory policies can be overriden.

-> **Note:** Sentinel runtime version pinning is supported only for Sentinel 0.23.0 and above, as well as Terraform Cloud agent versions 1.14.0 and above
