...
The following example policy parses Terraform plan and checks whether it includes security group updates to allow public ingress traffic from all CIDRs (`0.0.0.0/0`).
...
|  `refresh` | Boolean |  Whether the state was refreshed prior to the plan |
...
|  `replace_addrs` | An array of strings representing [resource addresses](/cli/state/resource-addressing) | The targets specified using the [`-replace`](/cli/commands/plan#replace-address) flag in the CLI or the `replace-addrs` property in the API. Undefined if no resource targets are specified. |
|  `speculative` | Boolean | Whether the plan associated with the run is a [speculative plan](/cloud-docs/run/#speculative-plans) only |
|  `target_addrs` |  An array of strings representing [resource addresses](/cli/state/resource-addressing). | The targets specified using the [`-target`](/cli/commands/plan#resource-targeting) flag in the CLI or the `target-addrs` property in the API. Undefined if no resource targets are specified. |
...
|  `created_at` | String | The time that the workspace was created. The timestamp follows the [standard timestamp format in RFC 3339](https://datatracker.ietf.org/doc/html/rfc3339). |
...
| Property Name |  Type  | Description |
|------ | --------- | ----------- |
|  `prior_monthly_cost` | String | The monthly cost estimate at the beginning of a plan. This value contains a positive decimal and can be `"0.0"`. |
| `proposed_monthly_cost` | String | The monthly cost estimate if Terraform applies the plan. This value contains a positive decimal and can be `"0.0"`. |
|  `delta_monthly_cost` |  String | The difference between the prior and proposed monthly cost estimates. This value may contain a positive or negative decimal and can be `"0.0"`. |
