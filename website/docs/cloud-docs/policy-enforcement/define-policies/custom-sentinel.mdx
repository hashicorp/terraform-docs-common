...
This topic describes how to create and manage custom policies using Sentinel policy language.  For instructions about how to use pre-written Sentinel policies from the registry, refer to [Copy pre-written Sentinel policies](/terraform/cloud-docs/policy-enforcement/define-policies/prewritten-sentinel)
...
1. You can also view Sentinel policy results in JSON format. Refer to [View Sentinel JSON results]((/terraform/cloud-docs/policy-enforcement/view-json-results) for additional information. 
