...
- README documentation included with each of the policy libraries.

Complete the following steps to implement pre-written Sentinel policies in your workspaces:

1. Obtain the policies you want to implement: You download policies directly into your repository or create a fork of the HashiCorp repositories. Alternatively, you can add the Terraform module to your configuration, which acquires the policies and connects them to your workspaces in a single step.
1. Connect policies to your workspace. If you downloaded policies or forked one or more repositories, you must manually connect the repository containing the policies to your HCP Terraform or Terraform Enterprise workspaces.       
...
- HCP Terraform
- Terraform Enterprise v202312-1 or newer

### Permissions

Your HCP Terraform or Terraform Enterprise user account must either be a member of the owners team or have the **Manage Policies** organization-level permissions enabled to create new policy sets and policies. Refer to the following topics for additional information:
...
- [policv-library-cis-aws-cloudtrail-terraform](https://github.com/hashicorp/policv-library-cis-aws-cloudtrail-terraform)
...
- Fork the HashiCorp policy GitHub repository
...
HashiCorp Sentinel policy libraries include a `sentinel.hcl` file. You can modify the file to customize your policy set. Refer to [Sentinel Policy Set VCS Repositories](/terraform/cloud-docs/policy-enforcement/manage-policy-sets/sentinel-vcs) for additional information.
...
1. Go to the [module in the Terraform registry](https://registry.terraform.io/modules/hashicorp-modules/aws-cis/sentinel/latest) and copy the code generated in the **Provision Instructions** tile.  
...
After successfully running the `terraform apply` command, Terraform evaluates Sentinel policies for each run of the workspaces you specified.

## Connect policies to your workspace

Skip this step if you [added the Terraform module](#add-the-terraform-module-to-your-configuration) to your configuration. When you use the module, you programmatically connect policies to workspaces in the HCL. 
...
1. If you stored the `sentinel.hcl` policy set file in folder, specify the path to the file in the **Policies path** field. The default is the root directory.
1. If you want to apply updated policies sets to the workspace from a specific branch, specify the name in the **VCS branch** field.  The default is the default branch configured for the repository.
