...
-> **Note:** Sentinel policies are available in the [Terraform Cloud Team & Governance tier](https://www.hashicorp.com/products/terraform/pricing).

You use the [Sentinel policy language](https://docs.hashicorp.com/sentinel/language) to define Sentinel policies. After you define policies, you must add them to [policy sets](/cloud-docs/policy-enforcement/manage-policy-sets) that Terraform Cloud can enforce on workspaces.
...
- [tfplan](/cloud-docs/policy-enforcement/sentinel/import/tfplan) - Access a Terraform plan, which is the file created as a result of the [`terraform plan` command](/cli/commands/plan).	The plan represents the changes that Terraform must make to reach the desired infrastructure state described in the configuration.
- [tfconfig](/cloud-docs/policy-enforcement/sentinel/import/tfconfig) - Access a Terraform configuration. The configuration is the set of `.tf` files that describe the desired infrastructure state.
- [tfstate](/cloud-docs/policy-enforcement/sentinel/import/tfstate) - Access the Terraform [state](/language/state). Terraform uses state to map real-world resources to your configuration.
- [tfrun](/cloud-docs/policy-enforcement/sentinel/import/tfrun) - Access data associated with a [run in Terraform Cloud](/cloud-docs/run/remote-operations). For example, you could retrieve the run's workspace.

You can create mocks of these imports to use with the the [Sentinel
CLI](https://docs.hashicorp.com/sentinel/commands) mocking and testing features. Refer to [Mocking Terraform Sentinel Data](/cloud-docs/sentinel/mock) for more details.
