...
You define Sentinel policies using the [Sentinel policy language](https://docs.hashicorp.com/sentinel/language). Then you can [add policies directly in the Terraform Cloud UI](/cloud-docs/policy-enforcement/manage-policy-sets), connect Terraform Cloud to a [VCS policy set](/cloud-docs/policy-enforcement/sentinel/vcs), or use the `tfe` provider's [`tfe_sentinel_policy` resource](https://registry.terraform.io/providers/hashicorp/tfe/latest/docs/resources/sentinel_policy) to upload policies using Terraform.
...
This page helps you start writing Sentinel policies for Terraform. We also recommend reviewing the [Sentinel Policy Language](https://docs.hashicorp.com/sentinel/language) documentation.
