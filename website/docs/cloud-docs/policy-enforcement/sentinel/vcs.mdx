---
page_title: VCS Policy Sets - Sentinel - Terraform Cloud
description: >-
 Prepare Sentinel policy sets in a version control repository to use in Terraform Cloud.
---

# Sentinel VCS Policy Sets
...
To enable policy enforcement, you must group Sentinel policies into policy sets and apply those policy sets to one or more workspaces.

One way to create policy sets is by connecting Terraform Cloud to a specially formatted version control repository containing Sentinel policies. When you push changes to policy files in the repository, Terraform Cloud automatically fetches the updated policy set. Refer to [Managing Policy Sets](/cloud-docs/policy-enforcement/manage-policy-sets) for more details.

A Sentinel policy set repository contains policy files, module files, and a a Sentinel [configuration file](#configuration-file) that defines the policy set. This page explains how to configure a policy set version control repository for Terraform Cloud policy enforcement.
...
A policy's `source` field can point to a file within the policy repository, or a remote source, such as a policy from HashiCorp's [foundational policies library](https://github.com/hashicorp/terraform-foundational-policies-library). Only HTTP and HTTPS remote sources are supported.
...
For GitHub, you must use the URL of the raw content of the policy. Using other types causes Terraform Cloud to error when checking the policy. For example, do not use `https://github.com/hashicorp/terraform-foundational-policies-library/master/cis/aws/networking/aws-cis-4.1-networking-deny-public-ssh-acl-rules/aws-cis-4.1-networking-deny-public-ssh-acl-rules.sentinel`.
...
For a more robust or flexible policy, you can expand the enforcement logic to also restrict provisioning to occur out of hours using the [time.hour](https://docs.hashicorp.com/sentinel/imports/time#time-hour) function.

## Modules

Terraform Cloud supports Sentinel's modules feature. This lets you write reusable policy code that you can import and use within several policies at once.

You can store modules locally or retrieve them from a remote HTTP or HTTPS source, similar to policies.

-> **Note:** We recommend reviewing [Sentinel runtime's modules documentation](https://docs.hashicorp.com/sentinel/extending/modules) to learn to use modules within Sentinel. The configuration examples in the runtime documentation pertain to the Sentinel CLI and not Terraform Cloud.

The following example module loads the code at `./modules/timezone.sentinel` relative to the policy set working directory and makes it available to other modules using the statement `import "timezone"`.
