...
One way to create policy sets is by connecting Terraform Cloud to a version control repository containing Sentinel policies. When you push changes to the repository, Terraform Cloud automatically uses the updated policy set. Refer to [Managing Policy Sets](/cloud-docs/policy-enforcement/manage-policy-sets) for more details.

A Sentinel policy set repository contains policy files, module files, and a Sentinel [configuration file](#configuration-file). 
