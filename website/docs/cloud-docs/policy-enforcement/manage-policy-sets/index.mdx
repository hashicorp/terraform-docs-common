...
If you meet the following requirements, HCP Terraform runs policy evaluations in your self-managed agent pool:

- You are on HCP Terraform **Premium** edition.
- You configure the workspace to run Terraform operations in a self-managed agent pool.
- You configure at lease one agent in the agent pool to accept `policy` jobs. Refer to the [HCP Terraform agent reference](/terraform/cloud-docs/agents/agents#accept) for more information.

If you do not meet all of the above requirements, policy evaluations run within the [HCP Terraform agent](/terraform/cloud-docs/agents) in HCP Terraform's infrastructure.
