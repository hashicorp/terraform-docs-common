...
When enabled, HCP Terraform periodically runs an automated health assessment for your workspace. Refer to [Health Assessment Scheduling](#health-assessment-scheduling) for details.
...
- **Other active runs:** The first health assessment starts during the next assessment period.
...
While an assessment is in progress, you cannot trigger another assessment. An on-demand assessment resets the scheduling for automated assessments, so the next automated assessment will during the next assessment period. 
