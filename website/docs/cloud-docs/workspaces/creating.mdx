...
- **No-Code Provisioning:** The no-code provisioning workflow creates a new workspace and deploys a [no-code ready module's](/cloud-docs/no-code-provisioning/provisioning) resources in it.

## Permissions

You must be a member of a team with [manage workspaces permissions](/cloud-docs/users-teams-organizations/permissions#manage-workspaces) to create a workspace.
