...
You must be a member of a team with [manage all projects & workspaces permissions](/cloud-docs/users-teams-organizations/permissions#manage-all-projects-workspaces), [manage workspaces only permissions](/cloud-docs/users-teams-organizations/permissions#manage-workspaces-only), or [admin permissions for a project](/cloud-docs/users-teams-organizations/permissions#project-admins) to create a workspace.
...
- [Learn more about running Terraform in your workspace](/cloud-docs/run/remote-operations): This includes how Terraform processes runs within the workspace, run modes, run states, etc.
...
After you manually start a run, Terraform Cloud automatically queues a plan when new commits appear in the selected branch of the linked repository or someone opens a pull request on that branch. Refer to [Webhooks](/cloud-docs/vcs#webhooks) for more details.
