...
1. (Version control only) Choose an existing version control provider or
   configure a new one. Refer to [Connecting VCS
   Providers](/terraform/cloud-docs/vcs) for more details.
1. (Version control only) Choose an organization (GitHub App only), and repository from the filterable list. The **Configure settings** page appears.
