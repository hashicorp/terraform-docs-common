...
After you create a new workspace from a version control repository, Terraform Cloud scans its configuration files for [Terraform variables](/terraform/cloud-docs/workspaces/variables#terraform-variables) and displays any that do not have a default value and do not have a definition in an existing global [variable set](/terraform/cloud-docs/api-docs/variable-sets). Terraform cannot perform successful runs in the workspace until you set values for these variables.
