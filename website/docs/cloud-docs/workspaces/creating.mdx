...
After you create a new workspace from a version control repository, Terraform Cloud scans its configuration files for [Terraform variables](/cloud-docs/workspaces/variables#terraform-variables) and displays them on the page. Terraform variables without default values or variables not defined by a [Variable Set](/cloud-docs/api-docs/variable-sets) will be displayed. Terraform cannot perform successful runs until you set values for these variables.

Choose one of the following actions:

- To skip this step, click **Go to workspace overview**. You can create these variables later from within the workspace.
- To create variables, enter a value for each variable. If a value is provided by another source (e.g., `auto.tfvars` file), click **Ignore variable**. Click **Save variables**.
