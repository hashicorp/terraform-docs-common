...
- **Triggers:** Whether to notify this destination for all events, or only for selected events. The following specific events are available:
  - **Triggers:** Whether to notify this destination for all events, or only for selected events. The following specific events are available:
    ...
    - **Drift**: If [drift detection](/cloud-docs/workspaces/settings/drift-detection) has been enabled, every time Terraform Cloud detects configuration drift.
    - **Drift Check Fail**: If drift detection has been enabled, every time a drift detection check fails.
...
For enabling and verifying configurations; refer to the next section for details.
