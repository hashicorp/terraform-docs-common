...
description: >-
  Learn how to use webhooks to notify external systems about run progress and other events. Create and enable workspace notifications in Terraform Cloud.
---
...
-> **Note:** Workspaces configured with `Local` [execution mode](/cloud-docs/workspaces/settings#execution-mode) do not support notifications.
...
- **Workspace Events**: Terraform Cloud can send notifications for all events or only for specific events. The following events are available:
  - **Drift**: Terraform Cloud detected configuration drift. This notification is only available if you enable [health assessments](/cloud-docs/workspaces/health) for the workspace.
