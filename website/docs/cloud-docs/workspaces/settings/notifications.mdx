...
1. Complete the following fields to configure your notifications:
   ...
   - **Workspace Events:** You can notify this destination for all events or only for specific events. The following events are available.
       - **Drift**: Every time Terraform Cloud detects configuration drift. Only if [Drift Detection](/cloud-docs/workspaces/settings/drift-detection) has been enabled for the workspace.
       - **Drift Check Fail**: Every time a drift detection check fails. Only if [Drift Detection](/cloud-docs/workspaces/settings/drift-detection) has been enabled for the workspace.
