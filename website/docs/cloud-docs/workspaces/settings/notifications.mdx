...
1. Configure the notifications:
   ...
   - **Run Events:** Terraform Cloud can send notifications for all events or only for specific events. The following events are available:
       ...
       - **Needs Attention**: A plan has changes and Terraform requires user input to continue. This may include approving the plan or a [policy override](/cloud-docs/run/states#5-the-policy-check-stage).
       - **Applying**: A run enters the [Apply stage](/cloud-docs/run/states#6-the-apply-stage), where Terraform makes the infrastructure changes described in the plan.
