...
Terraform Cloud can use webhooks to notify external systems about [drift detection](/cloud-docs/workspaces/settings/drift-detection) and the progress of runs in a workspace. Each workspace has its own notification settings, and can notify up to 20 destinations.
