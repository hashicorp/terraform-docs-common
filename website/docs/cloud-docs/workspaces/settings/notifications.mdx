...
Terraform Cloud can use webhooks to notify external systems about [drift detection](/cloud-docs/workspaces/settings/drift-detection) and the progress of runs. Each workspace has its own notification settings, and can notify up to 20 destinations.
...
1. Configure your notifications:
   ...
   - **Destination:** Terraform Cloud can deliver either a generic payload or a payload formatted specifically for Slack, Microsoft Teams, or an Email. Refer to [Notification Payloads](#notification-payloads) for details.

   - **Name:** A display name to use for this notification configuration.

   - **Webhook URL** This is only available for generic, Slack, and Microsoft Teams webhooks. The destination URL for the webhook payload. This URL must accept HTTP or HTTPS `POST` requests and should be able to use the chosen payload type.
   ...
   - **Token** (Optional) This is only available for generic webhooks. An arbitrary secret string that Terraform Cloud will use to sign its notification webhooks. Refer to [Notification Authenticity][inpage-hmac] for details. This setting is encrypted for storage, so you cannot view it after saving.
   ...
   - **Run Events:** You can notify this destination for all events or only for specific events. The following events are available:
       - **Created**: A run is created and enters the "Pending" state.
       - **Planning**: A run acquires the lock and starts to execute.
       - **Needs Attention**: A plan has changes and is not auto-applied or requires a policy override. Terraform requires user input to continue.
       - **Applying**: A run begins the apply stage after a plan is confirmed or auto-applied.
