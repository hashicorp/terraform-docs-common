...
- **Workspace Events**: Terraform Cloud can send notifications for all events or only for specific events. The following events are available:
  - **Drift**: Terraform Cloud detected configuration drift. This notification is only available if [health assessments](/cloud-docs/workspaces/health) are enabled for the workspace.
  - **Check Failure:** Terraform Cloud detected one or more failed continuous validation checks. This notification is only available if health assessments are enabled for the workspace.
  - **Health Assessment Fail**: A health assessment failed. This notification is only available if health assessments are enabled for the workspace.
