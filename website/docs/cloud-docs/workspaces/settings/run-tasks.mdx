...
Run tasks perform actions between the [plan](/cloud-docs/run/states#the-plan-stage) and [apply](/cloud-docs/run/states#the-apply-stage) stages of a [Terraform run](/cloud-docs/run). Once all run tasks complete, the run ends based on the most restrictive enforcement level in each associated run task.
