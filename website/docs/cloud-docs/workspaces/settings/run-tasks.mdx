...
Terraform Cloud run tasks let you directly integrate third-party tools and services at certain stages in the Terraform Cloud run lifecycle. You can use run tasks to validate Terraform configuration files, analyze plan outputs before applying them, scan for security vulnerabilities, or perform other custom actions.

Each run task in a workspace can send information to an external service at [specific run stages](#understanding-run-tasks-within-a-run). The external service processes the information and lets Terraform Cloud know if that task passed or failed. Terraform Cloud then uses this information to determine if a run should proceed.
...
When a run task is triggered, it sends an API payload to the specied external service. This payload contains a collection of run-related information, including a callback URL which the service uses to return a pass or fail status to Terraform Cloud. Terraform Cloud uses this status response to determine if a run should proceed, based on the run task's enforcement settings.
