...
You can access a complete list of all run tasks in the [Terraform Registry](https://registry.terraform.io/browse/run-tasks). These run tasks include the [HCP Packer integration](/cloud-docs/integrations/run-tasks#hcp-packer-run-task) and run tasks that our HashiCorp Technology Partners develop and maintain.
...
Run tasks perform actions between the [plan](/cloud-docs/run/states#the-plan-stage) and [apply](/cloud-docs/run/states#the-apply-stage) stages of a [Terraform run](/cloud-docs/run/remote-operations). Once all run tasks complete, the run ends based on the most restrictive enforcement level in each associated run task.
