...
1. Select a Source
   ...
   - **Managed** Run task requests are initiated within the HCP Terraform's infrastructure. This is the default option.
   - **Agent** HCP Terraform will ensure that run task requests are initiated within your agents. Your organization must have the `private-run-tasks` [entitlement][] to use this option.
