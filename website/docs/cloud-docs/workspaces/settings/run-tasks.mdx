...
Run tasks perform actions between the [pre-plan](/cloud-docs/run/states#the-pre-plan-stage) and [apply](/cloud-docs/run/states#the-apply-stage) stages of a [Terraform run](/cloud-docs/run/remote-operations). Once all run tasks complete, the run ends based on the most restrictive enforcement level in each associated run task.
