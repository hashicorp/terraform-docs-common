...
1. When a workspace with overriding auto-destroy settings on an existing project's auto-destroy settings is moved to a project with no auto-destroy settings, the settings become regular auto-destroy settings.
