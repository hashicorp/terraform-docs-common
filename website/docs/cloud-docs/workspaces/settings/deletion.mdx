...
There are two main ways of configuring auto destroy for a workspace.
1. Schedule a run to destroy all resources in a workspace at a specific date and time.
1. Allow Terraform Cloud to the destruction of a workspace's infrastructure after a period of workspace inactivity.

This ephemeral workspace entitlement reduces your infrastructure
...
When configured for the first time, the auto destroy duration setting will display a date and time on which an auto destroy run is scheduled to be performed.
This auto destroy date will be updated upon subsequent runs.
Each run that updates state will push out the scheduled auto destroy time by the length of time configured in the duration setting.

After a manual or automatic destroy run is completed, Terraform Cloud will wait until further state updates to schedule a new auto destroy run.
