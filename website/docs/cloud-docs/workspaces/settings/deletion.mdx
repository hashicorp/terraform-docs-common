...
date and time. This ephemeral workspace option reduces your infrastructure
spend by helping you automatically destroy resources that you only need
temporarily, such as development environments.

If you perform a destroy operation in the workspace before the scheduled auto
destroy, Terraform Cloud will cancel the scheduled run. You can re-enable it to
schedule another future auto destroy run. 
...
You can schedule [notifications](/terraform/cloud-docs/workspaces/settings/notifications) to alert you 24 hours before an auto destroy run, and to report auto destroy run results.
