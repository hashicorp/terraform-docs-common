...
1. Go to the workspace and click **Settings > Version Control**. The **VCS Settings** page appears.
...
This option instructs Terraform to begin new runs only for changes that affected specified files and directories. This behavior also applies to [speculative plans](/cloud-docs/run/#speculative-plans) on pull requests.

You can use trigger patterns and trigger prefixes in the **Add path** field to specify groups of files and directories.
- **Trigger Patterns:** Use glob patterns to specify the files that should trigger a new run. For example, `/submodule/**/*.tf`, specifies all files with the `.tf` extension that are nested below the `submodule` directory. You can also use more complex patterns like `/**/networking/**/*`, which specifies all files in the `networking` folder at this level of the file path (e.g., `/submodule/service-1/networking/private/main.tf`). Refer to [TODO: create glob patterns page](link) to learn more about glob patterns.
...
-> **Note:** Terraform triggers runs on all attached workspaces if it does not receive a list of changed files or if that list is too large to process. When this happens, Terraform Cloud may show several runs with completed plans that do not result in infrastructure changes.

### Terraform Working Directory

TODO: Does this only apply when you are in local execution mode? I'm unsure when you would want to set this. Can we give users some context Did we take this out --> I don't see it in the demo video.
