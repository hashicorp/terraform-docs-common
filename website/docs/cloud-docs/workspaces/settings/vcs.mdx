...
For workspaces that have "Only trigger runs when files in specified paths change" selected", Terraform Cloud assumes that only some content in the repository is relevant to the workspace. Only changes that affect the relevant content will trigger a run. This behavior also applies to [speculative plans](/cloud-docs/run/#speculative-plans) on pull requests — Terraform Cloud won't queue plans for changes that aren't considered relevant.

By default, only the designated working directory is considered relevant.

You can adjust this behavior in few ways:

- **Specify trigger patterns.** Terraform Cloud will queue runs for every file that matches provided glob patterns.

  For example, if you specify `/submodule/**/*.tf`, Terraform cloud will trigger runs for all the files changed nested below the `submodule` directory which end up with the `.tf` extension.
  You could specify even more complex patterns such as `/**/networking/**/*` which will match all the files changed which have `networking` folder in the path, e.g. `/submodule/service-1/networking/private/main.tf`.
  To learn more about glob patterns check [TODO: create glob patterns page]
- **Specify trigger prefixes.** Terraform Cloud will queue runs for changes in any of the specified trigger directories matching the provided prefixes (including the working directory).
