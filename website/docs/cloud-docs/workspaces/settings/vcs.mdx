...
- **Custom Regular Expression:** You can define your own regex to be matched against tags.
 ...
 -> **Note:** When using the [hashicorp/tfe provider](https://registry.terraform.io/providers/hashicorp/tfe). Regular expressions often include a backslash character `\` that serve as an escape sequence within terraform. Replace individual backlashes `\` with two backlashes `\\`. Read more about Terraform [escape sequences](https://www.terraform.io/language/expressions/strings#escape-sequences).


| Format               |      API/UI            |   TFE Provider/Terraform    |
|----------------------|------------------------|-----------------------------|
| Semantic Versioning  |    `^\d+.\d+.\d+$`     |     `^\\d+.\\d+.\\d+$`      |
| Prefix Versioning    |    `\d+.\d+.\d+$`      |     `\\d+.\\d+.\\d+$`       |
| Suffix Versioning    |    `^\d+.\d+.\d+`      |     `^\\d+.\\d+.\\d+`       |
