...
-> **Note:** When managing your workspace with the [hashicorp/tfe provider](https://registry.terraform.io/providers/hashicorp/tfe/latest/docs/resources/workspace#tags_regex) and triggering runs through matching git tags is set, 
you will need to include an addtional `\` to escape the regex pattern. For example, when you want to trigger a run when git tag is published with semantic versioning: `tags_regex = "^\\d+.\\d+.\\d+$"`. 
Read more about Terraform [escape sequences](https://www.terraform.io/language/expressions/strings#escape-sequences).
