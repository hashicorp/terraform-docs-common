...
We support `glob` patterns to describe a set of triggers for automatic run triggering. Refer to [trigger patterns](#only-trigger-runs-when-files-in-specified-paths-change) for details.
