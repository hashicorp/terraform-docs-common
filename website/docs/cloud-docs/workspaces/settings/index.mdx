...
In addition to the main auto-apply setting (which affects runs created by the UI, API, CLI, and version control webhooks), Terraform Cloud also has a separate setting for runs created by [run triggers](/terraform/cloud-docs/workspaces/settings/run-triggers) from another workspace.
