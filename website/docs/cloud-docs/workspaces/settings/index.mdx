...
### Drift Detection
-> Note: Note: Drift detection is in beta and is available in the [Terraform Cloud Business tier](https://www.hashicorp.com/products/terraform/pricing).

When you enable drift detection, Terraform Cloud automatically runs a drift detection assessment for this workspace every 24 hours. The assessment checks the actual settings of your infrastructure against those tracked in your workspace's state file.

Refer to [Drift Detection](/cloud-docs/workspaces/settings/drift-detection) for details.
...
You can also use a workspace's "Actions" menu to manage the workspace's lock.
