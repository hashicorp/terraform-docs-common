...
-> **Note:** Health Asessments (formerly Drift detection) is in beta and is available in the paid **Business** tier. Refer to [Terraform Cloud pricing](https://www.hashicorp.com/products/terraform/pricing) for details and  [provide feedback](https://forms.gle/BwjgvrKQcLbgqK3f7) about how we can improve this feature.

The term health assessment is used to describe the regular, automatic inspection of a workspace to surface resource lifecycle concerns. Presently, this includes infrastructure drift detection but may include other concerns in the future.
...
Health assessments is a workspace-level setting. When enabled, Terraform Cloud will automatically run a health assessment for that workspace every 24 hours. The assessment checks the actual settings of your infrastructure against those tracked in your workspace's state file.
...
> **Note:** You may also enforce assessments across all workspaces in an organization from the [organization assessments settings](/cloud-docs/users-teams-organizations/organizations/#assessments)
