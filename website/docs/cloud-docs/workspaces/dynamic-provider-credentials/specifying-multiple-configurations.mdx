---
page_title: Specifying Multiple Configurations - Workspaces - HCP Terraform
description: >-
  Specifying multiple provider dynamic provider credentials configurations for the same workspace.
---
...
Each environment variable has a corresponding default variable which can be used to share values across multiple configurations.  In this way, values common to multiple configurations can be specified a single time, rather than repeatedly for each configuration.  If a conflicting environment variable is explicitly set for a configuration in addition to the default value, the explicit value is given precedence over the default value.

#### Example

In the [example above](/terraform/cloud-docs/workspaces/dynamic-provider-credentials/specifying-multiple-configurations#example), if each of your Vault configurations used the same underlying Vault instance, you could define the Vault address a single time using the `TFC_DEFAULT_VAULT_ADDR` variable, and omit `TFC_VAULT_ADDR` and `TFC_VAULT_ADDR_ALIAS1`.  If a third Vault configuration was added, this time using a different Vault instance, you could set `TFC_VAULT_ADDR_ALIAS2` to override the Vault address specifically for the `ALIAS2` configuration.
