...
| `TFC_VAULT_ENCODED_CACERT`             | A PEM-encoded CA certificate that has been Base64 encoded.                                     | This certificate will be used when connecting to Vault. May be required when connecting to Vault instances that use a custom or self-signed certificate. |
