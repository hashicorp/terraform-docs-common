...
~> **Important:** If you are self-hosting [Terraform Cloud agents](/terraform/cloud-docs/agents), ensure your agents use [v1.x.0](/terraform/cloud-docs/agents/changelog#1-14-2-12-19-2023) or above. To use the latest dynamic credentials features, [upgrade your agents to the latest version](/terraform/cloud-docs/agents/changelog).
...
You will need to create a service principal for HCP Terraform to assume during runs. Remember the ID of the service principle as you will need it for the next steps.
...
|---------------------------------------------|------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `TFC_HCP_PROVIDER_AUTH`                     | `true`                                                                                               | Requires **v1.x.0** or later if self-managing agents. Must be present and set to `true`, or HCP Terraform will not attempt to use dynamic credentials to authenticate to HCP.                                                                                                                                           |
| `TFC_HCP_RUN_PROVIDER_RESOURCE_NAME`        | The resource name of the workload identity provider that will be used to assume the service principal| Requires **v1.x.0** or later if self-managing agents. Optional if `PLAN_PROVIDER_RESOURCE_NAME` and `APPLY_PROVIDER_RESOURCE_NAME` are both provided. These variables are described [below](/terraform/cloud-docs/workspaces/dynamic-provider-credentials/hcp-configuration#optional-environment-variables) |
...
| Variable                                      | Value                                                                                                                                                                                                                                                                      | Notes                                                                                                                                     |
|-----------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------|
| `TFC_HCP_WORKLOAD_IDENTITY_AUDIENCE`          | Will be used as the `aud` claim for the identity token. Defaults to the provider resource name for the current run phase that was derived from the values you provided for `RUN_PROVIDER_RESOURCE_NAME`, `PLAN_PROVIDER_RESOURCE_NAME`, and `APPLY_PROVIDER_RESOURCE_NAME` | Requires **v1.x.0** or later if self-managing agents. This is one of the default `aud` formats that HCP accepts.                          |
| `TFC_HCP_PLAN_PROVIDER_RESOURCE_NAME`         | The resource name of the workload identity provider that will be used to assume the service principal for the plan phase of a run.                                                                                                                                         | Requires **v1.x.0** or later if self-managing agents. Will fall back to the value of `RUN_PROVIDER_RESOURCE_NAME` if not provided.        |
| `TFC_HCP_APPLY_PROVIDER_RESOURCE_NAME`        | The resource name of the workload identity provider that will be used to assume the service principal for the apply phase of a run.                                                                                                                                        | Requires **v1.x.0** or later if self-managing agents. Will fall back to the value of `RUN_PROVIDER_RESOURCE_NAME` if not provided.        |
