...
Click **Projects & workspaces**. HCP Terraform displays the workspaces the current user account has permission to read runs for. The project drawer lists any projects the user has access to. ([More about permissions.](/terraform/cloud-docs/users-teams-organizations/permissions))
...
You can create workspaces through the [HCP Terraform UI](/terraform/cloud-docs/workspaces/creating), the [Workspaces API](/terraform/cloud-docs/api-docs/workspaces), or the [HCP Terraform CLI integration](/terraform/cli/cloud).
