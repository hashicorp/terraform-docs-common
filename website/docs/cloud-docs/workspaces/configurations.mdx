...
For Terraform Enterprise customers upgrading from a previous version, the functionality has a backfill capability that will clean up space for historical runs in batches. Per organization, each time a run completes or a new configuration version is uploaded a batch of 100 configuration versions will be archived. This will gradually free up existing object storage space over time.
