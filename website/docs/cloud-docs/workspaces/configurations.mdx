...
- **With direct uploads.** You can use a variety of tools to directly upload configuration content to Terraform Cloud:
  ...
  - **API:** Terraform Cloud's API can accept configurations as `.tar.gz` files, which can be uploaded by a CI system or other workflow tools. See [The API-driven Run Workflow](/terraform/cloud-docs/run/api) for more information.
...
- Each workspace must [specify a Terraform working directory](/terraform/cloud-docs/workspaces/settings#terraform-working-directory), so Terraform Cloud knows which configuration to use.
...
- **Created with a connected VCS repository.** Terraform Cloud will automatically archive VCS configuration versions once all runs are complete and re-fetch the configuration files from VCS as needed for new runs. Historical VCS configuration versions that were created and stored before upgrading to Terraform Enterprise v202203-1 or later are archived in batches of 100, when new configuration versions are uploaded or runs are completed. 
