...
Variables in project-scoped variable sets are only applied to the workspaces in the specified projects.
...
For example, if you apply `A_Variable_Set` and `B_Variable_Set` to the same project, Terraform Cloud will use any conflicting variables from `A_Variable_Set`. This will be the case regardless of which variable set has been edited most recently; Terraform Cloud only considers the lexical ordering of variable set names when determining precedence.
...
- **Workspace-Specific:** `VAR1`. That means `VAR1` equals `h` for this run, overwriting the value in A_Variable_Set.

- **A_Variable_Set:** `KEY1`. That means `KEY1` equals `x` for this run, overwriting the value in B_Variable_Set.

- **B_Variable_Set:** `ACCESS_KEY`, `ACCESS_ID`, and `VAR2`. That means `VAR2` equals `z` for this run, overwriting the value in C_Variable_Set.

- **C_Variable_Set:** `KEY2`. That means `KEY2` equals `c` for this run. This is a global variable set with no overwritten values.
