...
The **Variables** page is not available for workspaces configured with `Local` [execution mode](/terraform/cloud-docs/workspaces/settings#execution-mode). Workspace variables defined in HCP Terraform are not evaluated in local execution mode.
...
Variable sets are not evaluated during Teraform runs for workspaces configured with `Local` [execution mode](/terraform/cloud-docs/workspaces/settings#execution-mode).
