...
Policies are the guardrails that prevent Terraform runs from performing dangerous actions. Policies are managed as parts of versioned policy sets, which allow individual policy files to be stored in a supported VCS provider or uploaded via the Terraform Cloud API.
Upon evaluation, policies will adhere to a predefined [enforcement level](#enforcement-levels).

### Sentinel

Sentinel policies are written using the Sentinel [language](https://docs.hashicorp.com/sentinel/concepts/language).

### OPA

OPA policies are written using the Rego [language](https://www.openpolicyagent.org/docs/latest/policy-language/).
...
Enforcement levels are used for defining behavior when policies fail to evaluate successfully.

### Sentinel

Sentinel provides three enforcement modes:
...
OPA provides two enforcement modes:
