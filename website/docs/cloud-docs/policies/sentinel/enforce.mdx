...
Refer to the [Managing Policies](/cloud-docs/policies/sentinel/manage-policies) documentation for more detail about enforcement.


## Sentinel

For Sentinel, all `hard-mandatory` must pass in order for the run to continue to the "Confirm & Apply" state. All `soft-mandatory` policies must pass or be overridden for the run to continue to the "Confirm & Apply" state.
