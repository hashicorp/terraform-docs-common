...
### ServiceNow Team Tokens

Team-scoped tokens can help you manage a team's access to things in HCP Terraform, but team tokens are limited within the ServiceNow context. At this time, only one API team token is reccommended, regardless of the the number of teams that will access ServiceNow. 

Teams,and their associated team tokens, can be assigned to specific projects - [team tokens documentation] (https://developer.hashicorp.com/terraform/cloud-docs/users-teams-organizations/api-tokens#team-api-tokens). Proper permissions are required (e.g. READ, WRITE) to provision a resource. 
