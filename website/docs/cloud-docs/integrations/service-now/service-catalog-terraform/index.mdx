...
First, the Terraform Admin will configure your HCP Terraform organization with a
dedicated team for the ServiceNow integration, and obtain a team API token for
...
4. If using the [VCS flow](/terraform/cloud-docs/integrations/service-now/service-catalog-terraform/service-catalog-config#vcs-configuration): 
    ...
    provider](/terraform/cloud-docs/vcs) to this Terraform organization. This provider will include one or more repositories with Terraform configurations. Any repository that could be connected to manually-created HCP Terraform workspace can also be used as a workspace template in the ServiceNow integration.
    2. On your organization's VCS provider settings page (Settings > VCS Providers), find the "OAuth Token ID" for the VCS provider or providers you will be using with the ServiceNow integration. HCP Terraform uses the OAuth token ID to identify and authorize the VCS provider. **Save the OAuth token ID for later.**
...
6. Provide the following information to the ServiceNow Admin:
    ...
    3. The hostname of your Terraform Enterprise instance, or of HCP Terraform. The hostname of HCP Terraform is `app.terraform.io`.
