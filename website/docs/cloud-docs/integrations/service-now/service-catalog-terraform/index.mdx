...
Terraform Cloud, this integration lets ServiceNow users order Service Items,
create workspaces, and perform Terraform runs using prepared Terraform
configurations hosted in VCS repositories, or leveraging [no-code
modules](/terraform/cloud-docs/no-code-provisioning/module-design) for
self-service provisioning. 

-> **Note:** The v1.x integration is deprecated, but
documentation for existing installations can be found
[here](/terraform/cloud-docs/integrations/service-now/service-catalog-terraform/service-now-v1)
...
token, the hostname of your Terraform Cloud instance, and details about no-code modules or version
...
|                                                                              | If using VCS repositories, retrieve the OAuth token IDs and repository identifiers that Terraform Cloud uses to identify your VCS repositories. If using a no-code flow, [create a no-code ready module in your organization's private registry](/terraform/cloud-docs/no-code-provisioning/provisioning). Learn more in [Configure VCS Repositories or No-Code Modules](/terraform/cloud-docs/integrations/service-now/service-catalog-terraform/service-catalog-config#configure-vcs-repositories-or-no-code-modules).                     |
...
| If using the VCS flow, configure the VCS repositories in ServiceNow.                                |                                                                                                                                            |
...
- If using the VCS flow, one or more [supported version control
...
- If using no-code provisioning, one or more [no-code modules created in your
  organization's private
  registry](/terraform/cloud-docs/no-code-provisioning/provisioning). Refer to
the [no-code module configuration](/terraform/cloud-docs/integrations/service-now/service-catalog-terraform/service-catalog-config#no-code-module-configuration)
...
4. If using the [VCS flow](/terraform/cloud-docs/integrations/service-now/service-catalog-terraform/service-catalog-config#vcs-configuration): 
    1. If you haven't yet done so, [connect a VCS
    provider](/terraform/cloud-docs/vcs) to this Terraform organization. This provider will include one or more repositories with Terraform configurations. Any repository that could be connected to manually-created Terraform Cloud workspace can also be used as a workspace template in the ServiceNow integration.
    2. On your organization's VCS provider settings page (Settings > VCS Providers), find the "OAuth Token ID" for the VCS provider or providers you will be using with the ServiceNow integration. Terraform Cloud uses the OAuth token ID to identify and authorize the VCS provider. **Save the OAuth token ID for later.**
    3. Identify one or more VCS repositories in the VCS provider containing Terraform configurations that will be provisioned by the ServiceNow Terraform integration. Take note of any Terraform or environment variables used by the repositories you select. **Save the Terraform and environment variables for later.**
5. If using the [no-code flow](/terraform/cloud-docs/integrations/service-now/service-catalog-terraform/service-catalog-config#no-code-module-configuration), create one or more no-code modules in the private registry of your Terraform Cloud. **Save the no-code module names for later.**
6. Provide the following information to the ServiceNow Admin:
    1. The organization name 
    2. The team API token
    3. The hostname of your Terraform Enterprise instance, or of Terraform Cloud. The hostname of Terraform Cloud is `app.terraform.io`.
    4. The no-code module name(s) or the OAuth token ID(s) of your VCS provider(s), and the repository identifier for each VCS repository containing Terraform configurations that will be used by the integration.
    5. Any Terraform or environment variables required by the configurations in the
