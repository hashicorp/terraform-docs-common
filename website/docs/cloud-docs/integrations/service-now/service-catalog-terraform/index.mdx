...
Because administrators of these services are not necessarily the same person, 
this documentation refers to a **ServiceNow Admin** and a **Terraform Admin**.

1. The Terraform Admin will configure your Terraform Cloud organization with
...
token, the hostname of your Terraform Cloud instance, and details about the version
control repositories containing Terraform configurations and required variables
to the ServiceNow Admin.

2. The ServiceNow Admin will install the Terraform ServiceNow Catalog
integration to your ServiceNow instance, and configure it using the team API
token and hostname.

3. The ServiceNow Admin will create a Service Catalog within ServiceNow
...
This table shows the individual steps of the process:  
...
the requested infrastructure and report the status back to ServiceNow.
...
- One or more [supported version control
  systems](/terraform/cloud-docs/vcs#supported-vcs-providers) (VCSs) with read access
...
1. [Create a team](/terraform/cloud-docs/users-teams-organizations/teams) for that 
...
1. If you haven't yet done so, [connect a VCS provider](/terraform/cloud-docs/vcs) to
