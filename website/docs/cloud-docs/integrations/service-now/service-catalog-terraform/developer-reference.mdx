...
manage Flows in the Flow Designer. For Workflows, navigate to All > Workflow Editor. Flows are a newer 
solution developed by ServiceNow and are generally preferred over Workflows.

The ServiceNow Example Flows can be found in the ServiceNow Studio > Flows (or All > Flow Designer). 
By default, the flows execute upon submitting an order request for the various
