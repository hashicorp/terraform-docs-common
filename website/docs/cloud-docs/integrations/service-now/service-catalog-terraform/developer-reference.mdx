...
| `tf_no_code_workspace`| Resources for Terraform no-code modules API requests       |
...
| Create Run                    | Creates/queues a new run in the HCP Terraform workspace.                                                                                                                   |
| Apply Run                     | Applies a run in the HCP Terraform workspace.                                                                                                                              |
| Provision Resources           | Creates a new HCP Terraform workspace (with auto-apply), creates/queues a run, applies the run when ready.                                                                     |
...
| Provision No-Code Workspace and Deploy Resources | Creates a new HCP Terraform workspace based on a no-code module configured in the private registry (with auto-apply), creates any variables, creates/queues a run, applies the run when ready.                                                |
...
| Update No-Code Workspace and Deploy Resources | Updates a previously created no-code workspace to new no-code provisioning settings when a new release of the no-code module is available. Updates variable values attached to the workspace and starts a new Terraform run.     
