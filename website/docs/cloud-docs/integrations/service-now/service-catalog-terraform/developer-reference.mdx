...
and **Workflows** as their primary process engines. To see which engine an item is using, open it 
...
manage **Flows** in the **Flow Designer**. To manage **Workflows**, navigate to **All > Workflow Editor**. **Flows** are 
a newer solution developed by ServiceNow and are generally preferred over **Workflows**.

You can find the ServiceNow Example Flows in the **ServiceNow Studio > Flows** (or **All > Flow Designer**). 
By default, Flows execute when someone submits an order request for a catalog item. Admins can modify the 
