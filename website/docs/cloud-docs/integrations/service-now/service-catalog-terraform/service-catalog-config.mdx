...
Version 2.4.0 of the Service Catalog for Terraform introduces the ability to set custom names for your Terraform workspaces. Previously, workspace names defaulted to the ServiceNow RITM numbers without an option to change them. Now, you can choose a custom name that the RITM number will automatically append to, such as: `prod-us-east-1_RITM0010231`. If you leave the name blank, it will continue to default to the RITM number.

Workspace names can include letters, numbers, dashes (`-`), and underscores (`_`), and should not exceed 90 characters.
Refer to our [workspace naming recommendations](/terraform/cloud-docs/workspaces/creating#workspace-naming) for best practices.

## Workspace Tags

Version 2.4.0 also allows you to add custom tags to Terraform workspaces. Use the new "Workspace Tags" field to provide a comma-separated list of tags that will be parsed and attached to the workspace in Terraform Cloud. 

Tags give admins an easier way to categorize, filter, and manage workspaces provisioned through the Service Catalog for Terraform.
We recommend that admins work with their end users to set naming conventions for tags to avoid variations like `ec2`, `aws-ec2`, `aws_ec2`.
