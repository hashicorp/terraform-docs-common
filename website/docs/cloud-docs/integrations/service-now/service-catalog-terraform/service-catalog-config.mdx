...
control system (VCS) repositories or no-code modules containing the Terraform configurations which
...
1. The no-code module name or the OAuth token ID and repository identifier for each VCS repository that
...
1. If using the VCS flow, configure at least one one VCS repository in ServiceNow.
...
Terraform have two configuration options: they can be associated with a VCS
provider repository or be backed by a [no-code module created in your
organization's private registry](/terraform/cloud-docs/no-code-provisioning/provisioning). Administrators determine which workspace type
end users can request from the Terraform Catalog. Below we outline the key
differences between the version control and no-code approaches to help guide
this decision.
...
After configuring one or more repositories in ServiceNow, the names of those
repositories will be available in the "VCS Repository" dropdown menu when
ordering new workspaces through the following items in the Terraform Catalog:
...
New in version 2.5, application administrators gain the ability to configure
...
request and update Items. Both utilize no-code modules from the private registry
in Terraform Cloud to enable end users to request infrastructure without writing
code.
...
The following Catalog Items allow you to build and manage workspaces powered by
no-code modules:
...
- **Update No-Code Workspace and Deploy Resources**: updates a previously
  created no-code workspace to new no-code provisioning settings, when a new
  version of the module is released; supplies updates variable values, runs and
  applies Terraform.

Administrators can skip configuring VCS repositories in ServiceNow when using
no-code modules flows. The only input needed in the Catalog request is the name
of the no-code module. 

Before utilizing a no-code module, it must be set up in the private registry of
your Terraform Cloud account. With this one-time configuration complete,
...
> **Hands On:** Try the [Use No-Code Ready Modules in the ServiceNow Service Catalog for Terraform tutorial]().
...
1. Create a new workspace and specifying the no-code module name or the VCS repository provided by ServiceNow.
...
Version 2.4.0 also allows you to add custom tags to your Terraform workspaces. Use the "Workspace Tags" field to provide a comma-separated list of tags that will be parsed and attached to the workspace in Terraform Cloud. 
