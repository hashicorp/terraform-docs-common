...
The latest release, version 2.8.0 extends support for the key-value pair tags while still also supporting flat string tags that were introduced in version 2.4.0. Use the "Workspace Tags" field to provide a comma-separated list of key-value pair tags in the format "env: prod, instance: test" that will be parsed and attached to the workspace in HCP Terraform.
