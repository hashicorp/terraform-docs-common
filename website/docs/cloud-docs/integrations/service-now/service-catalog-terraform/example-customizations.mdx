...
When making customizations to the app, ensure you switch to the "Terraform"
scope. This guarantees that all items you create are correctly assigned to that
scope. To change the scope, click the globe icon at the top right of the screen.
For detailed instructions on changing the scope, refer to the [ServiceNow
documentation](https://developer.servicenow.com/dev.do#!/learn/learning-plans/washingtondc/new_to_servicenow/app_store_learnv2_buildneedit_washingtondc_application_scope).
