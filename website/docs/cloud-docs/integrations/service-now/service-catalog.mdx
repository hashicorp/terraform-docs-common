...
If using v2.2.0 or above, the Service Catalog app allows you to set an [execution mode](/terraform/cloud-docs/workspaces/settings#execution-mode) for your Terraform workspaces. There are three modes to choose from:

 - The default value is "Remote", which performs runs on Terraform Cloud's infrastructure. 
 - Changing the execution mode to "Local" allows you to execute Terraform runs locally using the CLI workflow. 
 - Selecting "Agent" enables you to run Terraform operations on isolated, private, or on-premises infrastructure. This option requires an Agent Pool to be created in your Terraform Organization beforehand. Provide its ID in the "Agent Pool ID" field when you order a new Workspace through the Service Catalog.

-> **Note:** [Terraform Cloud Agents](https://developer.hashicorp.com/terraform/cloud-docs/agents) are available in the Terraform Cloud Business Tier.