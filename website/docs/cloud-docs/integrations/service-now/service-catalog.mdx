...
Starting from version 2.2.0, the Service Catalog app allows you to set the 
[execution mode](https://developer.hashicorp.com/terraform/cloud-docs/workspaces/settings#execution-mode) 
for your Terraform Workspaces.
...
Selecting "Agent" enables you to run Terraform operations on isolated, private, or on-premises infrastructure. 
This option requires an Agent Pool to be created in your Terraform Organization beforehand. 
Provide its ID in the "Agent Pool ID" field when you order a new Workspace through the Service Catalog.

-> **Note:** [Terraform Cloud Agents](https://developer.hashicorp.com/terraform/cloud-docs/agents) are 
available in the Terraform Cloud Business Tier.