...
1. [Create a team](/cloud-docs/users-teams-organizations/teams) for that organization called "ServiceNow", and ensure that it has the organization-level ["Manage Workspaces" permission](/cloud-docs/users-teams-organizations/permissions#manage-workspaces-only). You do not need to add any users to this team.
