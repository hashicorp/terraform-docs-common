...
- **Enhanced visibility**: The Service Graph Connector for Terraform updates CMDB dashboards with resources deployed in Terraform Cloud.
...
This section outlines two methods through which the integration can import your Terraform resources into CMDB. You can configure the application to pull all your resources in one batch. Alternatively, you can set up webhooks in your Terraform workspaces, which will notify your ServiceNow instance about new deployments.
...
Internally, the application uses a scheduled job as a helper to keep track of the incoming webhook requests. You can activate, configure, and view the history of all webhook imports by navigating to **Scheduled Imports** and selecting **SG-Terraform Scheduled Process State**. By default, the job is set to run every minute. You can change it to run more frequently if you wish to process the webhook requests even faster.
...
If you don't need some of the resources to be imported into CMDB, you can deactivate them in the **Select CMDB Classes to Map Source Data** section of the application's ETL record, and they will be ignored during import. Toggle the switch on the resource mapping record to deactivate or reactivate it.
...
The Service Graph Connector automatically marks the deleted resources as **Non-Operational** in the ServiceNow CMDB. After the destroy operation is completed in Terraform Cloud and the application's import job is finished in your ServiceNow instance, the **Operational Status** field of all resources removed from the Terraform state during the deletion process will be updated to **Non-Operational**.
