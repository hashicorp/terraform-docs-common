...
Refer to the [Run Task Integration API](/terraform/cloud-docs/api-docs/run-tasks/run-tasks-integration#structured-results) for the exact payload specification, as well as the [Run Task JSON Schema](/schema/structured-run-task-results-schema.json) for code generation and payload validation.
