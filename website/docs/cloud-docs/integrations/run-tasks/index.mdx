...
Once your server receives this payload, Terraform Cloud expects you to callback to the supplied `task_result_callback_url` using the `access_token` as an [Authentication Header](/terraform/cloud-docs/api-docs#authentication) with a [jsonapi](/terraform/cloud-docs/api-docs#json-api-formatting) payload of the form:

Refer to the [Run Task Integration API](/terraform/cloud-docs/api-docs/run-tasks/run-tasks-integration) for the exact callback specification.
...
Streamlined Run Task Reviews are enabled when the integration payload contains a key `capabilities.outcomes` set to `true`. Enabling this feature allows integrations to progressively send multiple detailed outcomes per Run Task execution. This format allows for a rich display of Run Taks results and an enhanced user experience in Terraform Cloud. Integrations are able to return arbitrary sets of labels, optionally color coded by severity, as well as Markdown text, presented directly in the Run page.

In addition to the usual documentation, a [JSON Schema](/terraform/schemas/structured-run-task-results-schema.json) is provided for code generation and payload validation.

Refer to the [Run Task Integration API](/terraform/cloud-docs/api-docs/run-tasks/run-tasks-integration#structured-results) for the exact payload specification.

### Legacy Run Tasks

A Run Task is considered "legacy" when it does not return an `outcomes` collection, and will not benefit from the rich user experience gained by providing outcomes for Streamlined Run Task Reviews.
