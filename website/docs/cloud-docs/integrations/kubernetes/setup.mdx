...
1. Generate an [user or team](/terraform/cloud-docs/users-teams-organizations/api-tokens#access-levels) API token with permissions to manage workspaces, plan and apply runs, and upload configurations within Terraform Cloud and save it to a file. These instructions assume you are using a file named `credentials`.
