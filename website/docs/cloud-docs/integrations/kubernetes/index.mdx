...
The `Project` controller allows managing Terraform Cloud [Projects](/terraform/tutorials/cloud/projects) via Kubernetes Custom Resources.
...
The `Project` controller allows managing the Team Access [permissions](/terraform/cloud-docs/users-teams-organizations/permissions#workspace-permissions).
...
The above example extends the previous one by granting `admin` access permissions to the `qa` team for managing this project.
