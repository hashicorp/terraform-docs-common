...
~> **Warning**: Version 1 of the HCP Terraform Operator for Kubernetes is **deprecated** and is no longer maintained. If you are installing the operator for the first time, refer to [Set up the HCP Terraform Operator for Kubernetes](/terraform/cloud-docs/integrations/kubernetes/setup) for more information.
