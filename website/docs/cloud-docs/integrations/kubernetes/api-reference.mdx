...
In `spec` only the field `Name` is allowed, the rest are used in `status`. [More information](/terraform/cloud-docs/agents).
...
| `triggers` _[NotificationTrigger](#notificationtrigger) array_ | The list of run events that trigger notifications. Triggers represent the different notifications that HCP Terraform can send as a run's progress transitions between different states. There are two categories of triggers:   - Health Events: `assessment:check_failure`, `assessment:drifted`, `assessment:failed`.   - Run Events: `run:applying`, `run:completed`, `run:created`, `run:errored`, `run:needs_attention`, `run:planning`. |
