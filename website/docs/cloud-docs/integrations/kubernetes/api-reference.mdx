...
HCP Terraform agents use secret tokens, called `AgentToken`s, to connect to the Terraform Cloud agent pool. In `spec` only the field `Name` is allowed, the rest are used in `status`. [More information](/terraform/cloud-docs/agents)
