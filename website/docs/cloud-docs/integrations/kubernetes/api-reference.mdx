...
| `terraformVersion` _string_ | The version of Terraform to use for this workspace.<br />If not specified, the latest available version will be used.<br />Must match pattern: `^\\d{1}\\.\\d{1,2}\\.\\d{1,2}$`<br />More information:<br />  - /cloud-docs/workspaces/settings#terraform-version |
| `workingDirectory` _string_ | The directory where Terraform will execute, specified as a relative path from the root of the configuration directory.<br />More information:<br />  - /cloud-docs/workspaces/settings#terraform-working-directory |
