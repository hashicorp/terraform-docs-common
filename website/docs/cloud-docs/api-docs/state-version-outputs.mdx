---
page_title: State version outputs API for HCP Terraform
description: >-
 Use the HCP Terraform API's `/state-version-outputs` endpoint to read the outputs from a Terraform state version and show the current outputs for a workspace.
---
