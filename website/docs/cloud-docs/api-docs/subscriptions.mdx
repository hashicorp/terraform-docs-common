...
An organization can subscribe to different [feature sets](/terraform/cloud-docs/api-docs/feature-sets), which represent the [pricing plans](/terraform/cloud-docs/overview) available in Terraform Cloud. An organization's [entitlement set](/terraform/cloud-docs/api-docs#feature-entitlements) is calculated using its subscription and feature set.
