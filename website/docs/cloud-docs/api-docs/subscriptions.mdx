...
tfc_only: true
description: >-
  Use the `/subscriptions` endpoint to access subscription information. Show subscription for an organization, and show subscription by ID using the HTTP API.
---
...
An organization can subscribe to different [feature sets](/cloud-docs/api-docs/feature-sets), which represent the [pricing plans](/cloud-docs/overview) available in Terraform Cloud. An organization's [entitlement set](/cloud-docs/api-docs#feature-entitlements) is calculated using its subscription and feature set.
