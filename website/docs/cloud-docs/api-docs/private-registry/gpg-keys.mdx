...
description: >-
  Use the `/gpg-keys` endpoint to manage the GPG keys used to sign hosted providers. List, add, get, update, and delete GPG keys using the HTTP API.
---
...
These endpoints are only relevant to hosted providers. When you [publish a hosted provider](/terraform/cloud-docs/registry/publish-providers) to the Terraform Cloud private registry, you must upload the public key of the GPG keypair used to sign the release. Refer to [Preparing and Adding a Signing Key](/terraform/registry/providers/publishing#preparing-and-adding-a-signing-key) for more details.
...
| Status  | Response                                                   | Reason                                                         |
| ------- | ---------------------------------------------------------- | -------------------------------------------------------------- |
| [201][] | [JSON API document][] (`type: "gpg-keys"`)                 | Successfully uploads a GPG key to a hosted provider           |
