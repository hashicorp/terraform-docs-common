...
Use the the module version management API endpoints to deprecate module versions <!-- BEGIN: TFC:only name:revoke -->, revoke module versions, <!-- END: TFC:only name:revoke --> and revert of status of module versions in your organization’s private registry.
