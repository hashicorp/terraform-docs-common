...
## Fetch data about a module’s deprecation 

The [private registry module API](/terraform/cloud-docs/api-docs/private-registry/modules#sample-registry-request-private-module) returns data about private registry modules, including a module’s deprecation information.

For example, if you want to know the deprecation status of v0.0.1 of the `aws` provider’s `consul` module in your `my-cloud-org` organization, you could perform this API call:
...
If the module was deprecated, your response should include a `deprecation` key with the details of that module version’s deprecation.
...
### Sample request to GET all module versions

To check the deprecation status of all of your module’s versions, you can perform an API call like the following:
...
Your response includes multiple versions, each deprecated module including a `deprecation` key listing the details of a module’s deprecation. If a module version has not been deprecated, the `deprecation` field returns `null`.
