...
You can add publicly curated providers from the [Terraform Registry](https://registry.terraform.io/) and custom, hosted providers to your Terraform Cloud private registry. The private registry stores a pointer to public providers so that you can view their data from within Terraform Cloud. This lets you clearly designate all of the providers that are recommended for the organization and makes them centrally accessible.
...
- [Authentication](/terraform/cloud-docs/api-docs#authentication) is handled the same as all other Terraform Cloud endpoints.
...
Use this endpoint to create both public and hosted providers:
...
- **Hosted providers:** The hosted provider record will be available in the organization's registry provider list immediately after creation, but you must [create a version and upload release assets](/terraform/cloud-docs/registry/publish-providers#publishing-a-provider-and-creating-a-version) before consumers can use it. The private registry does not automatically update hosted providers when you release new versions. You must add each new version with the [Create a Provider Version](/terraform/cloud-docs/api-docs/private-registry/provider-versions-platforms#create-a-provider-version) endpoint.
...
~> **Important:** For hosted providers, you must also create a version, a platform, and upload release assets before consumers can use the provider. Refer to [Publishing a Hosted Provider](/terraform/cloud-docs/registry/publish-providers) for more details.
...
| `data.attributes.namespace`     | string |         | The namespace of the provider. For hosted providers this is the same as the `:organization_name` parameter. |
...
### Sample Payload (Hosted Provider)
...
### Sample Response (Hosted Provider)
...
| `:namespace`         | The namespace of the provider. For hosted providers this is the same as the `:organization_name` parameter. |
...
### Sample Request (Hosted Provider)
