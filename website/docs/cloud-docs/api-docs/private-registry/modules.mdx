...
| `data.attributes.vcs-repo.oauth-token-id`     | string  |         | The VCS Connection (OAuth Connection + Token) to use as identified. You can obtain this ID from the [oauth-tokens](/terraform/cloud-docs/api-docs/oauth-tokens) endpoint. This value must not be specified if `github-app-installation-id` is specified. |
| `data.attributes.vcs-repo.github-app-installation-id` | string |  | The VCS Connection GitHub App Installation to use. This ID can be obtained from the user settings page. Requires previously authorizing the GitHub App and generating a user-to-server token. You can manage this from User Settings within Terraform Cloud. This value must not be specified if `oauth-token-id` is specified.                                                                                                                                                      |
