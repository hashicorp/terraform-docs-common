...
Creates a new registry module version. This endpoint only applies to private modules without a VCS repository and VCS-linked branch based modules; VCS-linked Tag based modules automatically create new versions for new tags. After creating the version for a non-VCS backed module, the module should be uploaded to the returned upload link.
...
| `data.attributes.commit_sha` | string |         | A valid sha for the module version to ingress        |
