...
description: >-
  Use the `/policy-sets` endpoint to manage groupings of Sentinel and OPA policies. List, show, create, and update policy sets and policy set versions, and attach and detach policies from workspaces, projects, and exclusions using the HTTP API.
---
...
| `data.relationships.exclusions.data[]`        | array\[object] | `[]`      | A list of resource identifier objects that defines which excluded workspaces and are associated with the policy set. These objects must contain `id` and `type` properties, and the `type` property must be `workspaces`. For example, `{ "id": "ws-FVVvzCDaykN1oHiw", "type": "workspaces" }`.                                                                                                                                                                                                                                               |
...
-> **Note:** The `data.relationships.projects`, `data.relationships.workspaces`, and `data.relationships.exclusions` objects refer to projects, workspaces and exclusions directly attached to the policy set. These keys are omitted for policy sets marked as global, which are implicitly related to all of the organization's workspaces.
...
| `data.relationships.exclusions`               | array\[object] | (previous value) | An array of references to excluded workspaces that the policy set should be assigned to. Sending an empty array clears all exclusions assignments. You can only specify this attribute when `data.attributes.global` is `false`.                                                                                      |                                                                                                                                          |
...
`POST /policy-sets/:id/relationships/excludedWorkspaces`
...
`DELETE /policy-sets/:id/relationships/excludedWorkspaces`

| Parameter | Description                                                                                                   |
| --------- | ------------------------------------------------------------------------------------------------------------- |
| `:id`     | The ID of the policy set to detach from excluded workspaces. Use the "List Policy Sets" endpoint to find IDs. |
...
| `exclusions`      | The workspaces excluded from to this policy set's enforcement.                                                                                                                                                 |
