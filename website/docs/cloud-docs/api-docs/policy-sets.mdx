...
| `data.relationships.workspaceExclusions.data[]`       | array\[object] | `[]`      | A list of resource identifier objects specifying which workspaces Terraform Cloud excludes from a policy set's enforcement. These objects must contain `id` and `type` properties, and the `type` property must be `workspaces`. For example, `{ "id": "ws-FVVvzCDaykN1oHiw", "type": "workspaces" }`.                                                                                                                                                                                                                                                            |
...
| `include`           | **Optional.** Enables you to include related resource data. Value must be a comma-separated list containing one or more of `projects`, `workspaces`, `workspaceExclusions`, `policies`, `newest_version`, or `current_version`. See the [relationships section](#relationships) for details.                                                     |
...
| `data.relationships.workspaceExclusions`               | array\[object] | (previous value) | An array of references to excluded workspaces that Terraform Cloud will not enforce this policy set upon. Sending an empty array clears all exclusions assignments.                                                                                                                                          |                                                                                                                                 |
...
`POST /policy-sets/:id/relationships/workspaceExclusions`
...
curl \
  ...
  --request POST \
  --data @payload.json \
  https://app.terraform.io/api/v2/policy-sets/polset-3yVQZvHzf5j3WRJ1/relationships/workspaceExclusions
```
...
`DELETE /policy-sets/:id/relationships/workspaceExclusions`
...
| `workspaceExclusions` | The workspaces excluded from this policy set's enforcement.                                                                                                                                    |
