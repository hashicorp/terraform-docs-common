...
You can create new policies with the [Policies API](/cloud-docs/api-docs/policies) and then manually add them to a policy set. You can also create a versioned policy set that you add to Terraform Cloud with the [Policy Set Versions API](#create-a-policy-set-version) or that Terraform Cloud retrieves from a version control system.
