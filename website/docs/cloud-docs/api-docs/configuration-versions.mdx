...
| `backing_data_soft_deleted`  | Terraform Enterprise has soft-deleted the backing files associated with this configuration version. You can restore the backing data associated with it before it is permanently deleted if need be. |
| `backing_data_permanently_deleted`  | Terraform Enterprise has permanently-deleted the backing files associated with this configuration version, and can no longer be restored. |
