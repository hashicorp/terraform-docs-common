...
| `backing_data_soft_deleted`  | The configuration version's backing data has been soft-deleted. You can restore the backing data associated with it before it is permanently deleted if need be. |
...
## Soft-Delete a Configuration Version

`POST /api/v2/configuration-versions/:external_id/actions/soft_delete_backing_data`

| Parameter                   | Description                                    |
| --------------------------- | ---------------------------------------------- |
| `external_id`  | The external ID of the configuration version to soft-delete. |

This endpoint notifies Terraform Enterprise to soft-delete backing files associated with this configuration version. This endpoint can only soft-delete configuration versions that were created with the API or CLI, are in an `uploaded` [state](#configuration-version-states), and are not the current configuration version. Otherwise, calling this endpoint will result in an error.

| Status  | Response                  | Reason(s)                                                                                                                                     |
| ------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| [200][] | none                      | Successfully initiated the soft-delete process.                                                                                               |
| [400][] | [JSON API error object][] | Transitioning state to `backing_data_soft_deleted` was unsuccessful.                                                                          |
| [404][] | [JSON API error object][] | Configuration version was not found or user not authorized.                                                                                    |
...
## Restore Previously Soft-Deleted Configuration Version

`POST /api/v2/configuration-versions/:external_id/actions/restore_backing_data`

| Parameter                   | Description                                    |
| --------------------------- | ---------------------------------------------- |
| `external_id`  | The external ID of the configuration version to restore back to its uploaded state if applicable. |

This endpoint notifies Terraform Enterprise to restore backing files associated with this configuration version. This endpoint can only restore configuration versions that are not in a `backing_data_permanently_deleted` [state](#configuration-version-states), and applicable configuration versions will be restored back to their uploaded state. Otherwise, calling this endpoint will result in an error.

| Status  | Response                  | Reason(s)                                                                                                                                     |
| ------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| [200][] | none                      | Successfully initiated the restore process.                                                                                                   |
| [400][] | [JSON API error object][] | Transitioning state to `uploaded` was unsuccessful.                                                                                           |
| [404][] | [JSON API error object][] | Configuration version was not found or user not authorized.                                                                                    |
...
`POST /api/v2/configuration-versions/:external_id/actions/permanently_delete_backing_data`

| Parameter                   | Description                                    |
| --------------------------- | ---------------------------------------------- |
| `external_id`  | The external ID of the configuration version to permanently delete. |

This endpoint notifies Terraform Enterprise to permanently delete backing files associated with this configuration version. This endpoint can only permanently delete configuration versions that were created with the API or CLI, are in an `backing_data_soft_deleted` [state](#configuration-version-states), and are not the current configuration version. Otherwise, calling this endpoint will result in an error.

| Status  | Response                  | Reason(s)                                                                                                                                     |
| ------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| [200][] | none                      | Successfully initiated the permanently delete process.                                                                                        |
| [400][] | [JSON API error object][] | Transitioning state to `backing_data_permanently_deleted` was unsuccessful.                                                                   |
| [404][] | [JSON API error object][] | Configuration version was not found or user not authorized.                                                                                    |
