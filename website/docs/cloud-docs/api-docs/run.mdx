...
Creating a run with `debugging_mode: true` will queue the run in debugging mode. This is equivalent to setting the `TF_LOG` environment variable to a value of `TRACE` for this run. See [Debugging Terraform](/terraform/internals/debugging) for more information.
...
| `data.attributes.debugging-mode`                   | bool                 | `false`                                              | When set to `true`, the plan is queued with a verbose logging level. |
