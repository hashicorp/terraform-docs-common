...
| `empty_apply`    | Specifies the run performed an apply with no changes to resources. This is most commonly used to [upgrade terraform state versions](/cloud-docs/workspaces/state#upgrading-state). |
...
| `data.attributes.variables`                        | array\[{key, value}] | (empty array)                                      | Specifies an optional list of run-specific variable values. Refer to [Run-Specific Variables](/cloud-docs/workspaces/variables/managing-variables#run-specific-variables) for details.    |
| `data.attributes.plan-only`                        | bool                 | (from configuration version)                       | Specifies if this is a [speculative, plan-only](/cloud-docs/run/modes-and-options#plan-only-speculative-plan) run that Terraform cannot apply. Often used in conjunction with terraform-version in order to test whether an upgrade would succeed. |
...
| `filter[operation]` | **Optional.** A comma-separated list of run operations; the result will only include runs that perform one of these operations. Options are listed in [Run Operations](/cloud-docs/api-docs/run#run-operations).                                                                                                                                 |
| `filter[status]`    | **Optional.** A comma-separated list of run statuses; the result will only include runs that are in one of these statuses. Options are listed in [Run States](/cloud-docs/api-docs/run#run-states).                                                                                                                                         |
| `filter[source]`    | **Optional.** A comma-separated list of run sources; the result will only include runs that came from one of these sources. Options are listed in [Run Sources](/cloud-docs/api-docs/run#run-sources).                                                                                                                                       |
