...
The run operation specifies the Terraform execution mode. You can reference the following list of possible operation modes and use them as [runs list](/terraform/cloud-docs/api-docs/run#list-runs-in-a-workspace) query parameters.

| Operation        | Description                                                                                                                                                                |
|------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `plan_only`      | The run does not have an apply phase. This is also called a [speculative plan](/terraform/cloud-docs/run/modes-and-options#plan-only-speculative-plan).                |
| `plan_and_apply` | The run includes both plan and apply phases.                                                                                                               |
| `save_plan`      | The run is a saved plan run; it can include both plan and apply phases, but won't become the workspace's current run unless it is confirmed for apply.                     |
...
You can use the following sources as [runs list](/terraform/cloud-docs/api-docs/run#list-runs-in-a-workspace) query parameters.
...
| `data.attributes.save-plan`                        | bool                 | false                                              | Specifies if this should be a saved plan run. Saved plan runs perform their plan and checks without becoming the workspace's current run (like a plan-only run), but can be confirmed for apply; once confirmed, they become the current run. When creating new [configuration versions](/terraform/cloud-docs/api-docs/configuration-versions) for saved plan runs, be sure to make them `provisional`. |
...
| `filter[operation]` | **Optional.** A comma-separated list of run operations; the result will only include runs that perform one of these operations. Options are listed in [Run Operations](/terraform/cloud-docs/api-docs/run#run-operations).                                                                                                                                 |
| `filter[status]`    | **Optional.** A comma-separated list of run statuses; the result will only include runs that are in one of these statuses. Options are listed in [Run States](/terraform/cloud-docs/api-docs/run#run-states).                                                                                                                                         |
| `filter[source]`    | **Optional.** A comma-separated list of run sources; the result will only include runs that came from one of these sources. Options are listed in [Run Sources](/terraform/cloud-docs/api-docs/run#run-sources).                                                                                                                                       |
