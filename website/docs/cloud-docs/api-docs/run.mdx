...
| `plan_queued`          | Terraform Cloud is waiting for its backend services to start the plan. Terraform Cloud is designed to always have run capacity, so this status is rare. Terraform Enterprise is self-hosted, so this status can be more common. |
