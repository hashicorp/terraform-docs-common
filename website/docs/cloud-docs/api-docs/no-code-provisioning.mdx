...
curl \
  ...
  --request POST \
  --data @payload.json \
  https://app.terraform.io/api/v2/organizations/hashicorp/no-code-modules/nocode-WGckovT2RQxupyt1/workspaces
```
...
## Update a Workspace to use a new No-Code module version

These endpoints update a Workspace to a new No-Code module version. They form a multi-part update process. Once an update is initiated, Terraform Cloud starts a new plan, containing the resources that will be added, deleted, and updated. When the plan completes, perform a request to confirm and apply the changes of the updated module.
...
This endpoint returns the plan details and status for updating a workspace to a new No-Code module version.
...
This endpoint is used for confirming an update and finalizing the update for a workspace to a new No-Code module version.
