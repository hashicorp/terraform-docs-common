...
The no-code provisioning API allows you to create, configure, and deploy Terraform modules in the no-code provisioning workflows within HCP Terraform. For more information on no-code modules, see [Designing No-Code Ready Modules](/terraform/cloud-docs/no-code-provisioning/module-design).

## Allow no-code provisioning of a module
...
To deploy a module using the no-code provisioning workflow, you must meet the following requirement:
...
You can use this API to enable no-code provisioning for a specific module. You can also use this API to set options for the allowed values of a variable for a no-code module in your organization.
...
Use this API to update the settings for the no-code provisioning of a module. You can use this API to update the following settings:
...
| Parameter | Description                                 |
| --------- | ------------------------------------------- |
| `:id`     | The unique identifier for a no-code module. |
...
This GET endpoint can optionally return related resources if you request them with [the `include` query parameter](/terraform/cloud-docs/api-docs#inclusion-of-related-resources).
...
| `data.attributes.auto_apply`                            | boolean |  `false`  | If `true`, HCP Terraform will automatically apply changes when a Terraform plan is successful, with some exceptions. |
...
1. Use this API to initiate the update. HCP Terraform will start a new plan, which describes the resources that will be added, updated, or removed from the workspace.
