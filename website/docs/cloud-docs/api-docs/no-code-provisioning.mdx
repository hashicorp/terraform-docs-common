---
page_title: No-code provisioning API endpoints for HCP Terraform
description: >-
  Use the HCP Terraform API's `/node-code-modules` endpoint to designate and configure no-code modules. You can also use this endpoint to deploy and upgrade no-code workspaces.
---
