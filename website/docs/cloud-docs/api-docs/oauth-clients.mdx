...
An OAuth Client represents the connection between an organization and a VCS provider. The connection can be globally accessed throughout the org (default) or be scoped to one or more [projects](/terraform/cloud-docs/workspaces/organize-workspaces-with-projects).
...
| `data.relationships.projects.data[]` | array\[object] | `[]`      | A list of resource identifier objects that defines which projects are associated with the oauth client. If `data.attributes.organization-scoped` is set to `false`, the OAuth client will only be available to the projects in this list. These objects must contain `id` and `type` properties, and the `type` property must be `projects`. For example, `{ "id": "prj-AwfuCJTkdai4xj9w", "type": "projects" }`. |
...
| `data.attributes.organization-scoped` | boolean | (previous value) | Whether or not the oauth client is scoped to all projects and workspaces in the organization. Defaults to `true`.                                                                      |
| `data.relationships.projects`    | array\[object] | (previous value) | An array of references to projects that the oauth client should be assigned to. Sending an empty array clears all project assignments.                                               |
...
## Attach an OAuth Client to projects

`POST /oauth-clients/:id/relationships/projects`

| Parameter | Description                                                                                        |
| --------- | -------------------------------------------------------------------------------------------------- |
| `:id`     | The ID of the oauth client to attach to projects. Use the "List OAuth Clients" endpoint to find IDs. |
...
| Key path | Type           | Default | Description                                                                                                                                                                                                                                                            |
| -------- | -------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `data[]` | array\[object] |         | A list of resource identifier objects that defines the projects the oauth client will be detached from. These objects must contain `id` and `type` properties, and the `type` property must be `projects`. For example, `{ "id": "prj-AwfuCJTkdai4xj9w", "type": "projects" }`. |
