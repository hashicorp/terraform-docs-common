...
An OAuth Client represents the connection between an organization and a VCS provider. The connection can be globally accessed throughout the org (default) or be scoped to one or more [projects](/terraform/cloud-docs/projects/managing).
