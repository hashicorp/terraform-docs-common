...
## 2023-07-26
* Introduce support for a `custom` `team_project` access level and various customizable permissions. There are `project-access` permissions that apply to the project itself as well as `workspace-access` permissions that apply to all workspaces in the project. For more information see the documentation on [Project Team Access](cloud-docs/api-docs/project-team-access).
