...
| `data.attributes.auto-destroy-activity-duration`| string  | none | Specifies how long each workspace in a project waits before automatically destroying its infrastructure. Specify a duration of up to four digits, greater than `0` and followed by either `d` for days or `h` hours. <br/><br/> For example, to queue destroy runs after fourteen days of inactivity set `auto-destroy-activity-duration: "14d"`. All future workspaces in this project inherit the default value. Refer to [Automatically destroy inactive workspaces](/terraform/cloud-docs/projects/managing#automatically-destroy-inactive-workspaces) for details. |
