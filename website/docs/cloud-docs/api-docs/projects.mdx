...
| `data.attributes.auto-destroy-activity-duration`| string  | none | Specifies how long Terraform should wait before automatically destroying infrastructure in a workspace due to inactivity. You can specify a duration up to four digits that is greater than `0` followed by either a `d` for days or `h` hours. For example, to queue destroy runs after fourteen days of inactivity set `auto-destroy-activity-duration: "14d"`. Refer to [Automatically destroy inactive workspaces](/terraform/cloud-docs/projects/managing#automatically-destroy-inactive-workspaces) for additional information. |
