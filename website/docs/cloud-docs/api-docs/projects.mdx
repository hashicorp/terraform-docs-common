...
| `data.attributes.setting-overwrites` | object |      |  The keys in this object are attributes that have project-level defaults. Each attribute key stores a boolean value which is `true` by default. To overwrite the default inherited value, set an attribute's value to `false`. For example, to unset `execution-mode` as the project-level default, you set `setting-overwrites.execution-mode = false`.  |
