...
-> **Note:** Sentinel and OPA policies are paid featurea, available as part of the **Team & Governance** upgrade package. [Learn more about Terraform Cloud pricing here](https://www.hashicorp.com/products/terraform/pricing).

[Policy as Code](/cloud-docs/policy-enforcement) is an embedded policy as code framework integrated with Terraform Cloud.

Policies are configured on a per-organization level and are organized and grouped into [policy sets](/cloud-docs/policy-enforcement/manage-policies#policies-and-policy-sets), which define the workspaces on which policies are enforced during runs. In these workspaces, the plan's changes are validated against the relevant policies after the plan step. (For details, see [Run States and Stages](/cloud-docs/run/states).)

This page documents the API endpoints to create, read, update, and delete the policies in an organization. Use of these endpoints provides a method to manually manage individual policies within Terraform Cloud. To view and manage the results of a specific run's policy check, use the [Runs API](/cloud-docs/api-docs/run).

~> **Note**: These endpoints are no longer the recommended way to use Sentinel features in Terraform Cloud. For Sentinel, we suggest using versioned policy sets, which allows integrating with VCS or uploading policy set data and configuration as a whole. Use the [policy sets API](/cloud-docs/api-docs/policy-sets) to leverage versioned policy sets.
...
| `data.attributes.kind`                  | string         |                                               | The kind of policy that will be created. Valid values are `sentinel` and `opa`                                                                                                                                                                                                                                                             |
...
| `data.attributes.enforce[].mode`        | string         | `hard-mandatory`(Sentinel) , `advisory` (OPA) | The enforcement level of the policy. Valid values are `"hard-mandatory"`, `"soft-mandatory"`, and `"advisory"` for Sentinel. For OPA, Valid values are `"mandatory"`and `"advisory"`. For more details, see [Managing Policies](/cloud-docs/policy-enforcement/manage-policies)                                                            |
...
This endpoint uploads code to an existing Sentinel/OPA policy.
...
This PUT endpoint requires the text of a valid Sentinel/OPA policy, with a `Content-Type` of `application/octet-stream`.

See [Defining Sentinel Policies](/cloud-docs/policies/sentinel/import) for details about writing Sentinel code.
See [Defining OPA Policies](/cloud-docs/policies/opa/opa-policy-guidelines) for details about writing OPA code.
...
| `data.attributes.description`    | string         | `null`                                        | A description of the policy's purpose. This field supports Markdown and will be rendered in the Terraform Cloud UI.                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `data.attributes.query`          | string         | `null`                                        | The OPA query to run. This is only valid for OPA policies                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
