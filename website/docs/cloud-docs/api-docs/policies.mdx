...
-> **Note:** Sentinel and OPA policies are available in the [Terraform Cloud **Team & Governance** tier](https://www.hashicorp.com/products/terraform/pricing), and OPA policies are in beta. OPA Policies are also not available in Terraform Enterprise.
...
You configure policy enforcement on an organization level by adding policies, organizing policies into larger [policy sets](/cloud-docs/policy-enforcement/manage-policies#policies-and-policy-sets), and applying those policy sets to one or more workspaces. Within the specified workspaces, Terraform Cloud checks each Terraform plan against the the policy set and displays the results in the UI.

This page documents the API endpoints to create, read, update, and delete individual policies in an organization. To manage policy sets, use the [Policy Sets API](/cloud-docs/api-docs/policy-sets). To manage the policy check results, use the [Runs API](/cloud-docs/api-docs/run).

-> **Note**: We no longer recommend using these endpoints to manage Sentinel policies in Terraform Cloud. For Sentinel, we recommend creating versioned policy sets with the [Policy Sets API](/cloud-docs/api-docs/policy-sets). Policy sets let you integrate with version control systems and upload policy set data and configuration together.
...
Refer to [Defining Sentinel Policies](/cloud-docs/policies/sentinel/import) for details about writing Sentinel code.
Refer to [Defining OPA Policies](/cloud-docs/policies/opa/opa-policy-guidelines) for details about writing OPA code.
