...
For Slack and Microsoft Teams notifications, the payload conforms to the respective webhook API, and will result in a message posted with informational attachments. (For an example, see [the Slack section](/cloud-docs/workspaces/settings/notifications#slack) and [the Microsoft Teams section](/cloud-docs/workspaces/settings/notifications#microsoft-teams) of the notifications docs.) For Generic notifications, the payload has several fields:
