...
description: >-
  Use the `notification-configurations` endpoint to interaction with notification configurations. List, show, create, update, verify, and delete configurations for a workspace using the HTTP API.
---
...
| ------------------ | ----------------------- | -------------------------------------------------------------------------------------------------------- |
| Created            | `"run:created"`         | A run is created and enters the [Pending stage](/cloud-docs/run/states#the-pending-stage)                                                    |
| Planning           | `"run:planning"`        | A run acquires the lock and starts to execute.                                                      |
| Needs Attention    | `"run:needs_attention"` | A plan has changes and Terraform requires user input to continue. This input may include approving the plan or a [policy override](/cloud-docs/run/states#the-policy-check-stage). |
| Applying           | `"run:applying"`        | A run enters the [Apply stage](/cloud-docs/run/states#the-apply-stage), where Terraform makes the infrastructure changes described in the plan.                            |
...
Workspace events include detailed information about workspace-level validation events like [health assessments](/cloud-docs/workspaces/health) if you enable them for the workspace. Much of the information provides details about the associated [assessment result](/cloud-docs/api-docs/assessment-results), which Terraform Cloud uses to track instances of continuous validation. Generic notifications for workspace events contain the following information:
