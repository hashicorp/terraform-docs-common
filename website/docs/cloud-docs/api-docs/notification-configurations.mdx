...
| Health Assessment Errored | `"assessment:failed`"   | A health assessment failed. This option is only available if you enable health assessments for the workspace.                                        |
| Auto Destroy Reminder | `"workspace:auto_destroy_reminder`"   | An automated workspace destroy run is imminent. |
| Auto Destroy Results | `"workspace:auto_destroy_run_results`"   | An automated workspace destroy run was attempted. |
...
Workspace events include detailed information about workspace-level validation events like [health assessments](/terraform/cloud-docs/workspaces/health) if you enable them for the workspace. Much of the information provides details about the associated [assessment result](/terraform/cloud-docs/api-docs/assessment-results), which Terraform Cloud uses to track instances of continuous validation.

The types of attributes returned in the payload details will vary depending on the type of `trigger_scope`.
...
Health assessment payloads will have information about resource drift and continuous validation checks.
...
The shape of data in auto destroy notification payloads may differ depending on the success of the created run. Specific cases are called out below.

###### Reminder

Reminders that a destroy run will be triggered at some point in the future.
...
The final result of the scheduled auto destroy run will include additional metadata about the run.
...
Run-specific values will be empty when Terraform Cloud was unable to create an auto destroy run.
