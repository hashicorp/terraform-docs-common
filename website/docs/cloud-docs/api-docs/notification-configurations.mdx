...
| Checks Failed      | `"assessment:check_failure"` | One or more continuous validation checks did not pass. This option is only available if you enabled drift detection for the workspace. Continuous validation is not available in Terraform Enterprise. |
