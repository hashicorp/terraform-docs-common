...
| Needs Attention    | `"run:needs_attention"` | A plan has changes and Terraform requires user input to continue. This may include approving the plan or a [policy override](/cloud-docs/run/states#5-the-policy-check-stage). |
| Applying           | `"run:applying"`        | A run enters the [Apply stage](/cloud-docs/run/states#6-the-apply-stage), where Terraform makes the infrastructure changes described in the plan.                            |
...
| Drifted            | `"assessment:drifted"`  | Terraform Cloud detected configuration drift. This is only available if [drift detection](/cloud-docs/workspaces/settings/drift-detection) is enabled for the workspace.                                                     |
| Drift Check Failed | `"assessment:failed`"   | A drift detection assessment failed. This is only available if [drift detection](https://www.terraform.io/cloud-docs/workspaces/settings/drift-detection) is enabled for the workspace.                                          |

## Notification Payload

The notification is an HTTP POST request with a detailed payload whose content depends on the type of notification configured.

For Slack and Microsoft Teams notifications, the payload conforms to the respective webhook API, and results in a notification message with informational attachments. Refer to [Slack Notification Payloads](/cloud-docs/workspaces/settings/notifications#slack) and [Microsoft Teams Notification Payloads](/cloud-docs/workspaces/settings/notifications#microsoft-teams)  for examples. For generic notifications, the payload varies based on the type of notification:
...
| Name                                         | Type   | Description                                                               |
| -------------------------------------------- | ------ | ------------------------------------------------------------------------- |
| `payload_version`                            | number | Currently always "2".                                                     |
| `notification_configuration_id`              | string | ID of the configuration which defined this notification.                  |
| `notification_configuration_url`             | string | URL of the Notification Configuration.                                    |
| `trigger_scope`                              | string | "assessment" for workspace notifications.                                 |
| `trigger`                                    | string | Value of the trigger which caused the notification to be sent.            |
...
| `details.new_assessment_result`              | object | The most recent Assessment Result which triggered the notification.       |
| `details.new_assessment_result.id`           | string | ID of the Assessment Result.                                              |
| `details.new_assessment_result.url`          | string | URL of the Assessment Result.                                             |
...
| `details.prior_assessment_result`            | object | The Assessment Result prior to the one which triggered the notification.  |
| `details.prior_assessment_result.id`         | string | ID of the Assessment Result.                                              |
| `details.prior_assessment_result.url`        | string | URL of the Assessment Result.                                             |
...
| `details.workspace_id`                       | string | ID of the workspace.                                                      |
