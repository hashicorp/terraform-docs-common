...
| `data.attributes.execution-mode`              | string  | (nothing) | Which [execution mode](/terraform/cloud-docs/workspaces/settings#execution-mode) to use. Valid values are `remote`, `local`, and `agent`. When set to `local`, the workspace will be used for state storage only. This value must not be specified if `operations` is specified.                                                                                                                                                                                                                                |
...
| `data.attributes.setting-overwrites`          | object  |           | Attributes that have organization-level defaults are defined as keys in this object. Each attribute key stores a boolean value that relates to whether or not they are overwriting the default. In order to set one of these attributes to use the inherited default, set it to false. For example, to set `execution-mode` as the organization default: `setting-overwrites.execution-mode = false`. The setting overwrites object is not necessary to include when overwriting the default.                   |
