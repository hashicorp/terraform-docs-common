...
| `search[wildcard-name]`| **Optional.** If specified, restricts results to workspaces with partial matching. For example, `search[wildcard-name]=*-prod` returns all workspaces ending in `-prod`.            |
