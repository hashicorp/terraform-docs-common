...
Safe deleting a workspace will first check if the workspace has resources under management, and if not it will delete the workspace.

It is considered "unsafe" to delete workspaces with resources under management, as deleting a workspace on Terraform Cloud will leave all infrastructure managed by the workspace orphaned from Terraform.

A workspace can be safe deleted via two endpoints, which behave identically. One refers to a workspace by its ID, and the other by its name and organization.
...
| [409][] | [JSON API error object][] | Workspace is not safe to delete                                       |

## Force Delete a workspace

Force deleting a workspace removes it without checking whether the workspace currently has [resources under management](/cloud-docs/workspaces/settings#deleting-a-workspace-with-resources-under-management). This ignores all of the safety checks present in a [safe delete](#safe-delete-a-workspace).

By default workspaces may only be force deleted by an organization owner. If the [Teams can delete workspaces with resources under management](/cloud-docs/users-teams-organizations/organizations#general) organization setting is enabled, workspace admins may also force delete.

!> **Warning:** Use caution when force deleting a workspace. Any infrastructure managed by the workspace will no longer be managed by Terraform. It is recommended that you apply a [Destroy Run](/cloud-docs/run/modes-and-options#destroy-mode) before deleting a workspace.

A workspace can be force deleted via two endpoints, which behave identically. One refers to a workspace by its ID, and the other by its name and organization.
