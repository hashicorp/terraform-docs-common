...
- `GET /workspaces/:workspace_id/relationships/tags`: Call this endpoint to [list tags workspace tags](#get-tags) stored as flat string values.
- `GET /workspaces/:id/tag-bindings`: Call this endpoint to [list workspace tags](#list-tags) stored as key-value pairs.  
- `GET /workspaces/:workspace_id/tag-bindings`: Call this endpoint to [list tags](#get-tags) stored as key-value pairs that were added directly to the workspace instead of inherited from the parent project. 
- `GET /workspaces/:workspace_id/effective-tag-bindings`: Call this endpoint to [list workspace tags](#get-tags) stored as key-value pairs that were added directly to the workspace as well as key-value tags inherited from the parent project.
...
- `DELETE /workspaces/:workspace_id/relationships/tags`: Call this endpoint to [delete workspace tags](#remove-tags-from-workspace) stored as flat string values.  To remove tags from a workspace stored as key-value pairs, use the [`PATCH /workspaces/:workspace_id`] endpoint.
...
| `data.relationships.tag-bindings`            | array of objects  | `[]`          | A list
...
| `data.relationships.tag-bindings.data` | object  | none | Specifies the type of relationship the workspace has with the attached tags stored as key-value pairs. This value must be `tag-binding` to bind tags to the workspace. |
| `data.relationships.tag-bindings.data.attributes` | object  | none | Specifies a key name and value to attach to the workspace as a tag. |
| `data.relationships.tag-bindings.data.attributes.key` | string | none | Specifies the name of the key to use for a tag. |
| `data.relationships.tag-bindings.data.attributes.value` | string | none | Specifies the name of the value to use for a tag. |
...
| `data.relationships.tag-bindings` | array | none | Specifies a list of objects that define tags stored as key-value pairs to attach to the workspace. |
| `data.relationships.tag-bindings.data` | object | none | Specifies a set of tags stored as key-value pairs to attach to the workspace.  |
| `data.relationships.tag-bindings.data.type` | object | none | Specifies the type of relationship the workspace has with the attached tags stored as key-value pairs. This value must be `tag-binding` to bind tags to the workspace.  |
| `data.relationships.tag-bindings.data.type.key` | string | none | Specifies the name of the key to use for a tag.  |
| `data.relationships.tag-bindings.data.type.value` | string | none | Specifies the value of of the `key` to use for a tag.  |
...
- `GET /workspaces/:workspace_id/relationships/tags`: Lists tags stored as flat string values.
- `GET /workspaces/:workspace_id/tag-bindings`: Lists tags stored as key-value pairs.
- `GET /workspaces/:workspace_id/effective-tag-bindings`: Lists tags stored as key-value pairs that are inherited from the parent project.

### Path parameters

| Parameter       | Description                                                                                                                                                       |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `:workspace_id` | The workspace ID to fetch single-value tags for. Obtain this from the [workspace settings](/terraform/cloud-docs/workspaces/settings) or the [Show Workspace](#show-workspace) endpoint. |

This endpoint returns the single-value tags that are currently associated to a workspace.
...
The following request returns tags stored as flat string values:
...
```
The following request returns tags stored as key-value pairs:
...
The following request returns tags stored as key-value pairs that are attached to the parent project and inherited by the workspace:
...
The following response lists tags stored as flat string values:
...
The following response lists tags stored as key-value pairs:
...
The following response lists tags stored as key-value pairs that are attached to the parent project and inherited by the workspace:
...
Use the following endpoint to add flat string value tags to an existing workspace:

`POST /workspaces/:workspace_id/relationships/tags`

To add tags stored as key-value pairs to an existing workspace, call the `PATCH /workspaces/:workspace_id` and provide workspace tags in the JSON payload. Refer to [Update a workspace](#update-a-workspace) for additional information.

You can also bind tags stored as key-value pairs when creating a workspace. Refer to [Create a workspace](#create-a-workspace) for additional information.
