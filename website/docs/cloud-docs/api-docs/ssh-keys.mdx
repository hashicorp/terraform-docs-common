...
- They can be assigned to VCS provider integrations ([available in the API as `oauth-tokens`](/terraform/cloud-docs/api-docs/oauth-tokens)). Azure DevOps Server and Bitbucket Server require an SSH key. Other providers only need an SSH key if your repositories include submodules that are only accessible via SSH (instead of your VCS provider's API).
