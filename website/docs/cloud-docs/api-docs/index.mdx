...
- `opa` — Allows an organization to use [Open Policy Agent (OPA)][] to create and enforce policies on Terraform runs. Affects the [policies][], [policy sets][], and [policy checks][] endpoints.
...
[Open Policy Agent (OPA)]: /cloud-docs/policy-enforcement#opa
