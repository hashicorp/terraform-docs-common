...
-   `sso` — Allows an organization to manage and authenticate users via single sign on.
    <!-- BEGIN: TFC:only -->
    For Terraform Cloud, refer to [SSO](/cloud-docs/users-teams-organizations/single-sign-on).
    <!-- END: TFC:only -->
    For Terraform Enterprise, refer to [SAML SSO](/enterprise/saml/configuration).
