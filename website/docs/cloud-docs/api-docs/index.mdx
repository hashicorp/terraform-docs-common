...
- `global-run-tasks` — Allows an organization to apply [run tasks](/terraform/cloud-docs/workspaces/settings/run-tasks) to every workspace in an organization. Affects the [run tasks][] endpoints. This feature is currently in beta.
