...
Policy Set Parameters let you inject parameters into an existing policy. This set of APIs covers create, update, list and delete operations on parameters.

Sentinel allows a policy authors to supply parameters to help facilitate policy reuse and ensure sensitive values do not need to be hard-coded in a policy.
