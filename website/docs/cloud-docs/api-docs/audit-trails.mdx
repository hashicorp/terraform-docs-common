...
[Policy evaluations](/terraform/enterprise/policy-enforcement/manage-policy-sets) run in the Terraform Cloud Agent in Terraform Cloud's infrastructure. Policy evaluartions have a lifecycle that includes the following events:
  * `queue`
  * `run`
  * `pass`
  * `fail`
...
You can create Terraform Cloud [run tasks](/terraform/cloud-docs/workspaces/settings/run-tasks) at an organization level to directly integrate third-party tools and services at certain stages in the Terraform Cloud run lifecycle. You can use run tasks to validate Terraform configuration files, analyze execution plans before applying them, scan for security vulnerabilities, or perform other custom actions.
...
A Terraform Cloud organization emits this event when someone in your organization [associates a new run task with a workspace](/terraform/cloud-docs/workspaces/settings/run-tasks#associating-run-tasks-with-a-workspace) (`workspace_task`). Alongside the [standard audit trail fields](#standard-response-fields), this event response includes the following fields:
...
A Terraform Cloud organization emits this event when someone in your organization updates a workspace's associated run task (`workspace_task`). Alongside the [standard audit trail fields](#standard-response-fields), this event response includes the following fields:
...
A Terraform Cloud organization emits this event when someone in your organization destroys a workspace's associated run task (`workspace_task`). Alongside the [standard audit trail fields](#standard-response-fields), this event response includes the following fields:
