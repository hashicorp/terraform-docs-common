...
[Policy evaluations](/terraform/enterprise/policy-enforcement/policy-results#view-policy-results) run in the Terraform Cloud agent in HCP Terraform's infrastructure. Policy evaluations have a lifecycle that includes the following events:
  * `queue`
  * `run`
  * `pass`
  * `fail`
