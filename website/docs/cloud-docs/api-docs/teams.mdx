...
description: >-
  Requests and responses for managing teams and their organization-level permissions.
---
...
-> **Note:** You cannot set `manage-workspaces` to `false` when setting `manage-projects` to `true`, since project permissions cascade down to workspaces.
