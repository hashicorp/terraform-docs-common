...
| `data.attributes.json-state`         | string  | (nothing) | **Optional** Base64 encoded json state, as expressed by `terraform show -json`. See [JSON Output Format](https://www.terraform.io/internals/json-format) for more details                                                                                                                  |
...
-> **Note**: The `hosted-state-download-url` attribute provides a URL from which you can download the raw, internal state. When Terraform 1.3+ is used, the `hosted-json-state-download-url` attribute provides the same state, but in a [stable format](https://www.terraform.io/internals/json-format).
