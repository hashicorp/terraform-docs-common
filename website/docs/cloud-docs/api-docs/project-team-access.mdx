...
-> **Note**: A `team-project` resource represents a team's _local_ permissions on a specific project. Teams can also have _organization-level_ permissions that grant access to projects, and Terraform Cloud uses whichever access level is higher. (For example: a team with the "manage projects" permission has admin access on all projects, even if their `team-project` on a particular project only grants read access.) For more information, see [Managing Project Access](/terraform/cloud-docs/users-teams-organizations/teams#managing-project-access).
...
| `custom`        | Custom access permissions on projects and project workspaces.                                                                                                    |
...
-> **Note:** As mentioned in [Add Team Access to a Project](#add-team-access-to-a-project) and [Update Team Access
to a Project](#update-team-access-to-a-project), several permission attributes are not editable unless `access` is
...
level for the project settings and workspace runs permissions are `read`. Conversley when the access level is `admin`, the implicit level for the project settings is `delete` and the workspace runs permission is `apply`. To see all of the implied permissions at the different access levels, see the [Implied Custom Permission Levels](#implied-custom-permission-levels)
...
| `data.attributes.workspace-access.runs`                   | string  | "none"  | If `access` is `custom`, the permission to grant for the project's workspaces' runs. Can only be used when `access` is `custom`. Valid values include `none`, `read`, `plan`, or `apply`.                                                    |
| `data.attributes.workspace-access.sentinel-mocks`         | string  | "none"  | If `access` is `custom`, the permission to grant for the workspace's Sentinel mocks. Can only be used when `access` is `custom`. Valid values include `none`, or `read`.                                                     |
| `data.attributes.workspace-access.state-versions`         | string  | "none"  | If `access` is `custom`, the permission to grant for the workspace's state versions. Can only be used when `access` is `custom`. Valid values include `none`, `read-outputs`, `read`, or `write`.                                                    |
...
| `data.attributes.workspace-access.move`                   | boolean | false  | If `access` is `move`, this permission allows the team to move workspaces into and out of the project.        |
...
As mentioned above, when setting team access levels of `read`, `write`, `maintain`, or `admin`, the following permissions that can be set individually when using an access level of `custom`, are given values based on the access level.
Below is each access level, and the implicitly granted custom permission

-> **Note** that when using `custom` as the access levels, any permission not specified will be given the default level listed in [Add Team Access to a Project](#add-team-access-to-a-project).


| Permissions for `read`              | level  | 
|-------------------------------------|--------|
| project-access.settings             | "read" |
| project-access.teams                | "none" |
| workspace-access.runs               | "read" |
| workspace-access.sentinel-mocks"    | "none" |
| workspace-access.state-versions     | "read" |
| workspace-access.variables          | "read" |
| workspace-access.create             | false  |
| workspace-access.locking            | false  |
| workspace-access.move               | false  |
| workspace-access.run-tasks          | false  |


| Permissions for `write`             | level   | 
|-------------------------------------|---------|
| project-access.settings             | "read"  |
| project-access.teams                | "none"  |
| workspace-access.runs               | "apply" |
| workspace-access.sentinel-mocks"    | "read"  |
| workspace-access.state-versions     | "write" |
| workspace-access.variables          | "write" |
| workspace-access.create             | false   |
| workspace-access.locking            | true    |
| workspace-access.move               | false   |
| workspace-access.run-tasks          | false   |


| Permissions for `maintain`          | level   | 
|-------------------------------------|---------|
| project-access.settings             | "read"  |
| project-access.teams                | "none"  |
| workspace-access.runs               | "apply" |
...
| workspace-access.state-versions     | "write" |
| workspace-access.variables          | "write" |
| workspace-access.create             | true    |
| workspace-access.locking            | true    |
| workspace-access.move               | false   |
| workspace-access.run-tasks          | true    |


| Permissions for `admin`             | level     | 
|-------------------------------------|-----------|
| project-access.settings             | "delete"  |
| project-access.teams                | "manage"  |
| workspace-access.runs               | "apply"   |
| workspace-access.sentinel-mocks"    | "read"    |
| workspace-access.state-versions     | "write"   |
| workspace-access.variables          | "write"   |
