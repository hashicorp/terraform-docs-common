...
For details on the implied permissions of each access level, see [Implied Custom Permission Levels](#implied-custom-permission-levels)
...
| `data.attributes.workspace-access.dekete`                | boolean | false  | If `access` is `custom`, the permission granting the ability to delete the project's workspaces. Can only be used when `access` is `custom`.                                                   |
