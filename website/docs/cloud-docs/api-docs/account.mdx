...
For internal reasons, team tokens and organization tokens are associated with a synthetic user account called a "service user." For account requests authenticated by a team token or organization token, Terraform Cloud returns the associated service user. To access the underlying team or organization associated with the token, you may use the `authenticated-resource` relationship. (For user tokens,
