...
Account represents the current user interacting with Terraform. It returns the same type of object as the [Users](/terraform/cloud-docs/api-docs/users) API, but also includes an email address, which is hidden when viewing info about other users. For requests authenticated by a team token or organization token, the service
user associated with those objects will be returned. To access the underlying token holder, you may use the `authenticated-resource` relationship. For user tokens,
this will be the same as the user itself.
