---
page_title: Policy evaluations API for HCP Terraform
description: >-
  Use the `/policy-evaluations` endpoint to read policy outcomes and evaluations from Sentinel and OPA policies that HCP Terraform performs during a Terraform run.
---
