...
description: >-
  Use the `/policy-evaluations` endpoint to manage the Sentinel and OPA policy evaluations performed on a Terraform run. List and show polices and policy outcomes, override policies, and list policy evaluations using the HTTP API.
---
