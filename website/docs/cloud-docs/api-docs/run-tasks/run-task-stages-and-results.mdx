...
| `awaiting_override` | The task stage is waiting for user input. Once overridden, the run returns to the `running` state.                  |
...
| `policy-evaluation`  | Information about the policy evaluation for a task-stage. |
