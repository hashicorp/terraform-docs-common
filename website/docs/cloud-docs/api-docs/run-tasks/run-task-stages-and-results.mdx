...
Terraform Cloud triggers [run tasks](/cloud-docs/workspaces/settings/run-tasks) in the [Post-Plan Stage](/cloud-docs/run/states#3-the-post-plan-stage) of a Terraform run, which occurs after Terraform completes the plan. As run tasks progress, Terraform Cloud uses run task stages to report their status. Each stage status produces a results object that contains details about the run task's operations. These run task results determine whether the run task passes or fails.

This page lists the endpoints you can use to retrieve run task stages and the results from those stages. Refer to the [Run Tasks API](https://www.terraform.io/cloud-docs/api-docs/run-tasks) for endpoints to create and manage run tasks within Terraform Cloud. Refer to the [Run Tasks Integration API](https://www.terraform.io/cloud-docs/api-docs/run-tasks-integration) for endpoints necessary to build custom run tasks for the Terraform Cloud ecosystem.
