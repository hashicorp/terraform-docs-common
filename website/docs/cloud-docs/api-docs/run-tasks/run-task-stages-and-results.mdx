...
| `awaiting_override` | Indicates that the task stage is waiting for user input, once overridden it re-enters back into the `running` state |
...
| `policy-evaluation`  | Additional information about the policy evaluation for a task-stage. |
