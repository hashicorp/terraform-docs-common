...
| `relationships.outcomes.data`     | array | (Optional) A collection of detailed run task outcomes |
...
When surfacing run task outcomes with tags labeled "severity", or "status", those tags will be used to enrich the list display of outcomes in Terraform Cloud, enabling earlier feedback of severity and statuses if desired.
