...
| `tags.key`     | string |The name of the header tag. This should be a very short name ideally no more than two or three words. Special handling is given to `severity` and `status` keys. |
| `tags.key[].label`     | string | The text value of the tag. |
| `tags.key[].level`     | enum string | (Optional) The error level for the tag. Defaults to `none`. Either `none`, `info`, `warning`, or `error`. For levels other than `none`, labels are rendered with color and icon for that level. |

##### Severity and Status Tags

When surfacing run task outcomes with tags "severity", or "status", those tags will be used to enrich the list display of outcomes in Terraform Cloud, enabling earlier feedback of severity and statuses.
