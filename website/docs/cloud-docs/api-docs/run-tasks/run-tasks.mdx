...
description: >-
  Use HCP Terraform API's `/tasks` endpoint to read, create, update, and delete run tasks, and read, update, delete and associate run tasks to workspaces.
---
...
| `data.relationships.agent-pool.data.id`                  | string |         | (Optional) The agent pool used to originate requests for the run task. Requires the `private_run_tasks` feature entitlement.                                                                       |
