...
[JSON API Schema document]: /schema/run-tasks/runtask-results.json
...
You may also configure the run task to begin during a specific [run stage](/terraform/cloud-docs/run/states). Run tasks use the [Post-Plan Stage](/terraform/cloud-docs/run/states#the-post-plan-stage) by default.
