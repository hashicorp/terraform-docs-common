...
| `workspace_count` | `number` | The number of workspaces using this terraform version. |
| `workspaces`      | `string` | A comma-separated list of workspaces using this module. |
...
query string parameters using key-value pairs. The parameter key defines the
filter's target field and operator, and the parameter value contains the filter
value to be used during the query.

Filters can be combined by providing multiple filter query string parameters
separated with `&`. When multiple filters are used, a logical AND is used
to evaluate the results.

Each filter key string must use the following format:

```
filter[<FILTER_INDEX>][<FIELD_NAME>][<FIELD_OPERATOR>][FIELD_VALUE_INDEX]
```
...
- **FIELD_OPERATOR**: The operator to use when filtering. Must be a supported by
the field type being filtered. See [Filter Operators](#filter-operators) for the available operators and
their supported field types.
- **FIELD_VALUE_INDEX**: Must be `0`. This parameter is reserved for future use.
...
_Show workspaces that contain `test` in their name_
(View Type: `workspace`)
...
_Show data for all workspaces_ (View Type: `workspace`)
...
_Show data for all workspaces with `test` in their name _ (View Type: `workspace`)

```shell
curl \
  --header "Authorization: Bearer $TOKEN" \
  https://app.terraform.io/api/v2/organizations/acme-corp/explorer?type=workspaces&filter[0][workspace_name][contains][0]=test
```
