...
Filters can be combined. When multiple filters are used, a logical AND is used
...
the field type being filtered. See [Filter Operators] for the available operators and
...
| `is_before`        | `datetime`                    | Filters data to rows where the field value is earlier than the filter value. ISO 8601 formatted dates
are recommended for filter values. If no time zone offset is provided, the filter value will be interpreted as UTC. |
| `is_after`         | `datetime`                    | Filters data to rows where the field value is earlier than the filter value. ISO 8601 formatted dates
are recommended for filter values. If no time zone offset is provided, the filter value will be interpreted as UTC. |
