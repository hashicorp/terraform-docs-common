...
| `data.attributes.default-agent-pool-id`                                   | string  | (previous value) | Required when `default-execution-mode` is set to `agent`. The ID of the agent pool belonging to the organization. This value must not be specified if `execution-mode` is set to `remote` or `local`                                                                                                                    |
...
| `default-agent-pool`  | An agent pool that is set as the organization's default, will only be set if the `default-execution-mode` is `agent`.         |
