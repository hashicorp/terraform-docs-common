...
| `data.attributes.cost-estimation-enabled`                                 | boolean | true      | Whether or not the cost estimation feature is enabled for all workspaces in the organization. Defaults to true. In Terraform Enterprise, Cost Estimation must also be enabled in Site Administration.                                          |
