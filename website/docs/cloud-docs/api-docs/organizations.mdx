...
| `data.attributes.aggregated-commit-status-enabled`                        | boolean | false     | Whetger or not to enable aggregated VCS status updates for triggered workspaces. This can be useful for monorepo projects with configuration that spans across many workspaces.    |
