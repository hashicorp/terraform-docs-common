...
| `data.relationships.default-project.data.id`                              | string  |          | Specifies the ID of the default project. If set to `null`, Terraform removes the organization's default project. If omitted, Terraform doesn't change the organization's default project. You must have [owner permissions](/terraform/cloud-docs/users-teams-organizations/permissions#organization-owners) to update the default project.                                        |
