...
| `data.attributes.assessments-enforced`                                     | boolean | (false)   | Whether or not to compel health assessments for all workspaces capable of being assessed. When true, health assessments will occur on all compatible workspaces, regardless of the value of the workspace setting `assessments-enabled`. When false, health assessments will only occur for workspaces that opt in by setting `assessments-enabled: true`.
