...
| `data.attributes.aggregated-commit-status-enabled`                        | boolean | false     | Whether or not to enable aggregated VCS status updates for triggered workspaces. This can be useful for monorepo projects with configuration that spans across many workspaces. You can not specify this option as true if `send-passing-statuses-for-untriggered-speculative-plans` is true.   |
