...
| `data.relationships.allowed-workspaces.data.id`   | string |         | The ID of the workspace that is allowed to use the agent pool. Refer to [Scoping Agent Pools to Specific Workspaces](/cloud-docs/agents#scoping-agent-pools-to-specific-workspaces). Available in Terraform Enterprise v202207-1 and later. |
