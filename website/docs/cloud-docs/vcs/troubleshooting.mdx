...
### Clicking "Connect organization `<X>`" with Bitbucket Data Center raises an error message in Terraform Cloud

Terraform Cloud uses OAuth 1 to authenticate the user to Bitbucket Data Center. The first step in the authentication process is for Terraform Cloud to call Bitbucket Data Center to obtain a "request token". After the call completes, Terraform Cloud redirects you to Bitbucket Data Center with the request token.

An error occurs when Terraform Cloud calls to Bitbucket Data Center to obtain the request token but the request is rejected. Some common reasons for the request to be rejected are:

- The API endpoint is unreachable; this can happen if the address or port is incorrect or the domain name doesn't resolve.
- The certificate used on Bitbucket Data Center is rejected by the Terraform Cloud HTTP client because the SSL verification fails. This is often the case with self-signed certificates or when the Terraform Enterprise instance is not configured to trust the signing chain of the Bitbucket Data Center SSL certificate.
...
- Verify that the Terraform Cloud client accepts the HTTPS connection to Bitbucket Data Center. This can be done by performing a `curl` from the Terraform Enterprise instance to Bitbucket Data Center; it should not return any SSL errors.
- Verify that the Consumer Key, Consumer Name, and the Public Key are configured properly in Bitbucket Data Center.
- Verify that the HTTP URL and API URL in Terraform Cloud are correct for your Bitbucket Data Center instance. This includes the proper scheme (HTTP vs HTTPS), as well as the port.

### Creating a workspace from a repository hangs indefinitely, displaying a spinner on the confirm button

If you were able to connect Terraform Cloud to Bitbucket Data Center but cannot create workspaces, it often means Terraform Cloud isn't able to automatically add webhook URLs for that repository.
...
### Terraform Cloud returns 500 after authenticating with the VCS provider (other than Bitbucket Data Center)
...
1. Confirm the reauthorization. Terraform Cloud redirects you to your VCS Provider where you can reauthorize the connection.
