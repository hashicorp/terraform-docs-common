...
### Failure to retrieve configuration version

If Terraform Cloud displays an error about configuration version retrieval, the VCS provider is likely not responding to Terraform Cloud's requests. 

```
Error: failed to retrieve configuration
Retrieving the configuration version failed with the error: Timed out while fetching the configuration version..
```
This can happen if there is a network problem between Terraform Cloud and the VCS provider. Push a new commit with any content to a branch in the problem repository to trigger a new configuration version. New commits trigger the repository's associated webhook, creating a new run and configuration version in Terraform Cloud.
