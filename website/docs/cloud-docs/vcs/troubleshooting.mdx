...
When configuring status checks with Azure DevOps the web interface may auto populate Genre and Name fields (beneath "Status to check") with incorrect values that do not reflect what Terraform Cloud is sending.
...
In the example below the status check is named `Terraform Cloud/paul-hcp/paul-test-1` and needs to be configured with Genre `Terraform Cloud` and Name `paul-hcp/paul-test-1`.

![Azure DevOps screenshot: configuring required status checks correctly](/img/docs/ado-required-status-check-after.png)
