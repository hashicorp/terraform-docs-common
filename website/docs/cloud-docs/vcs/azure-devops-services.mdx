...
1. On your Microsoft Entra admin cente, create a New Microsoft Entra Application. Provide the callback URL and get the application ID and key.
...
1. In a new browser tab, login to your [Microsoft Entra admin cente](https://entra.microsoft.com/#home), and open the page to register a new MS Entra App by navigating to this [link](https://entra.microsoft.com/#view/Microsoft_AAD_RegisteredApps/CreateApplicationBlade/quickStartType~/null/isMSAApp~/false). If you have access to multiple Entra tenants, switch to the tenant in which you want to register the application.

   ~> **Important:** According to Microsoft's [announcement](https://devblogs.microsoft.com/devops/no-new-azure-devops-oauth-apps/), the creation of new OAuth applications for Azure DevOps is no longer permitted. Instructions to register MS Entra Oauth App are [here](following these [instructions](https://learn.microsoft.com/en-us/entra/identity-platform/quickstart-register-app#register-an-application).
...
1. Click the register button to create the app and navigate to its information page.
1. Once the app is registered, navigate to its API permissions tab, and add **vso.code** and **vso.code_status** permissions.
...
Ensure that the Azure DevOps organization you intend to link with HCP Terraform is connected to the correct Microsoft Entra tenant by following these [instructions](https://learn.microsoft.com/en-us/azure/devops/organizations/accounts/connect-organization-to-azure-ad?view=azure-devops)
