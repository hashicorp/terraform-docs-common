...
This page explains the five main steps required to connect HCP Terraform to your Azure DevOps Services VCS:
...
## Step 2: From your Microsoft Entra admin cente, Create a New Microsoft Entra Application

1. In a new browser tab, login to your [Microsoft Entra admin center](https://entra.microsoft.com), and open the page to register a new MS Entra App by navigating to this [link](https://entra.microsoft.com/#view/Microsoft_AAD_RegisteredApps/CreateApplicationBlade/quickStartType~/null/isMSAApp~/false). If you have access to multiple Entra tenants, switch to the tenant in which you want to register the application.

   ~> **Important:** According to Microsoft's [announcement](https://devblogs.microsoft.com/devops/no-new-azure-devops-oauth-apps/), the creation of new OAuth applications for Azure DevOps Services is no longer permitted. Instructions to register MS Entra Oauth App are provided [here](following these [instructions](https://learn.microsoft.com/en-us/entra/identity-platform/quickstart-register-app#register-an-application).

1. A form appears asking for your company and application information.
...
1. Fill out the fields and checkboxes with the corresponding values currently displayed in your HCP Terraform browser tab. HCP Terraform lists the values in the order they appear and includes controls for copying values to your clipboard. Here is an example:
   ...
   | Redirect URI               | `Web` followed by `https://app.terraform.io/<YOUR CALLBACK URL>`               |

1. Click the register button to create the app.
1. Once the app is registered, navigate to the **API permissions** tab, and add **vso.code** and **vso.code_status** permissions.
...
Ensure that the Azure DevOps organization you intend to link with HCP Terraform is connected to the correct Microsoft Entra app created in step 2by following these [instructions](https://learn.microsoft.com/en-us/azure/devops/organizations/accounts/connect-organization-to-azure-ad?view=azure-devops).
