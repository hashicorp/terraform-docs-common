...
**As from Terraform Enterprise v202309-1 we recommend customers to be on at least v8.0 of Bitbucket server/data Center as versions prior to this may fail due to an incomplete implementation of RFC 8332 in MINA SSHD used by Bitbucket, refer to [Apache MINA SSHD documentation](https://issues.apache.org/jira/browse/SSHD-1216) for more details.
...
1. Create an SSH key pair. SSH keys must have an empty passphrase because Terraform Cloud cannot use SSH keys that require a passphrase.
...
1. When prompted, confirm that you wish to use the URL as entered. If you specified Terraform Cloud's main URL, click **Continue**.  If you specified an organization URL, enable the **Use this URL** option and then click **Continue**.
...
Complete the following steps in Terraform Cloud or Terraform Enterprise to request access to Bitbucket and add the SSH private key.
