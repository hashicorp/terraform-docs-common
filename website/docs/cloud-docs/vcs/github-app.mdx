...
After initial configuration of GitHub App is performed, and therefore VCS provider instance created, GitHub App option will become grayed out and unavailable. You can modify existing configuration without removing it from GitHub, and re-creating.
To add new repositories, use GitHub repository Settings tab(must be repository owner for it to be visible and accessible) -> Github Apps -> Terraform Cloud -> Configure -> Repository access
In order to pull up the pop-up window again, use HCP Terraform -> Workspaces -> New -> Version Control Workflow -> GitHub App -> click GitHub Organization right above the repository list on the left -> Add another organization. 
