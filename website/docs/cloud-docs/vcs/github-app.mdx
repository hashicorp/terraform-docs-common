...
-> **Note:** This VCS Provider is only available on Terraform Cloud. If you are using Terraform Enterprise, you can follow the instructions for creating [GitHub App for TFE](/terraform/enterprise/admin/application/github-app-integration) or connecting to [GitHub.com via OAuth](/terraform/cloud-docs/vcs/github).
...
You can use the Terraform Cloud GitHub App to create workspaces and private registry modules from the UI, the API, or the TFE Terraform provider.  The following tools can use any version of Terraform Cloud to access these features, but require a minimum version of Terraform Enterprise:

- If using the UI, any version of TFC or TFE v202302-1 and later is required.
- If using the API, Terraform Cloud or TFE v202303-1 and later is required.
- If using [go_tfe](https://github.com/hashicorp/go-tfe), version 1.19.0 or later of go_tfe and either Terraform Cloud or TFE v202303-1 and later are required.
- If using [tfe_provider](https://registry.terraform.io/providers/hashicorp/tfe/latest/docs), version v0.43.0 or later of the tfe_provider and either Terraform Cloud or TFE v202303-1 and later are required.
