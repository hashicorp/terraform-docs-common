...
This topic describes how to connect Bitbucket Cloud to Terraform Cloud. Bitbucket Cloud is the cloud-hosted version of Bitbucket. For self-hosted Bitbucket Data Center instances, refer to [Configuring Bitbucket Data Center Access](/terraform/cloud-docs/vcs/bitbucket-data-center). Refer to [Connecting VCS Providers to Terraform Cloud](/terraform/cloud-docs/vcs) for other supported VCS providers.
...
- In the following steps, you must provide Terraform Cloud with the private key. Although Terraform Cloud does not display the text of the key to users after it is entered, it retains it and will use it when authenticating to Bitbucket Cloud.
