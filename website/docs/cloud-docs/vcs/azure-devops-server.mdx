...
These instructions are for using an on-premises installation of Azure DevOps Server for HCP Terraform's VCS features. [Azure DevOps Services has separate instructions,](/terraform/cloud-docs/vcs/azure-devops-services) as do the [other supported VCS providers.](/terraform/cloud-docs/vcs)
