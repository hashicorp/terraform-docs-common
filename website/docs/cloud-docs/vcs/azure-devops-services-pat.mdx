...
If you do not intent to use personal access tokens to connect to Azure DevOps Services, refer to [Set up the Azure DevOps Server VCS provider](/terraform/cloud-docs/vcs/azure-devops-server). For information about other supported VCS providers, refer to [Connect to VCS Providers](/terraform/cloud-docs/vcs).
...
The account you use for connecting HCP Terraform must have **Project Collection Administrator** access enabled for any projects containing repositories of Terraform configurations. This permission is required to create webhooks. You cannot create custom access roles with lower levels of privilege because Microsoft does not allow delegation of this capability.
...
- Personal access tokens have a maximum lifetime of one year. When the token expires, HCP Terraform cannot to connect to Azure DevOps Services until the token is replaced. To avoid a gap in service, do one of the following actions before the token expires:
...
Follow security best practices and your organization's policies for protecting important credentials when handing and storing the private key. Someone with access to the key can use it to push code to the repositories you use to manage your infrastructure.
...
1. On a secure workstation, create an SSH keypair that HCP Terraform can use to connect to the Azure DevOps Services domain. The following example uses the `ssh-keygen` command to create a `service_terraform` file with the private key and a `service_terraform.pub` file with the public key:
  ...
  The SSH key must have an empty passphrase. HCP Terraform cannot use SSH keys that require a passphrase.
