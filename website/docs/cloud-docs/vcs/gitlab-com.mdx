...
These instructions are for using GitLab.com for Terraform Cloud's VCS features. [GitLab CE and GitLab EE have separate instructions,](/terraform/cloud-docs/vcs/gitlab-eece) as do the [other supported VCS providers.](/terraform/cloud-docs/vcs)
...
- In the following steps, you must provide Terraform Cloud with the private key. Although Terraform Cloud does not display the text of the key to users after it is entered, it retains it and will use it when authenticating to GitLab.
