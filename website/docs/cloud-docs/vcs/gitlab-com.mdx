...
1. This page has a list of applications and a form for adding new ones. The form has two text fields and some checkboxes.
   ...
   -> **Note:** In Gitlab v15 the `Expire access tokens` control was removed. It is no longer necessary for Terraform Cloud, or Terraform Enterprise versions `v202205-1` and later. Terraform Enterprise versions from `v202105-1` until `v202205-1` would refresh Gitlab OAuth tokens every 6 hours.
