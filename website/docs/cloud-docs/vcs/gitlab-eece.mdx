...
1. This page has a list of applications and a form for adding new ones. The form has two text fields and some checkboxes.
   ...
   -> **Note:** Previous versions of Terraform Cloud required disabling `Expire access tokens` because Gitlab auth tokens would only be refreshed after 6 hours, but Gitlab would expire them after 2 hours. The setting does not exist on newer Gitlab versions.
