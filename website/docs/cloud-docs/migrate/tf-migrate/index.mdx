...
The `tf-migrate` tool supports creating HCP Terraform workspaces configured to use the CLI or the VCS workflow. 

If your local configuration is a Git repository and has either GitHub or GitLab configured as a remote, `tf-migrate` asks you to confirm that you would like to create a VCS-driven workspace. If you have more than one VCS connection configured in your HCP Terraform organization, `tf-migrate` also asks you to choose which VCS connection to use.

If your local configuration is not a Git repository, or does not use one of the supported remotes, `tf-migrate` automatically creates a CLI-driven workspace instead.
