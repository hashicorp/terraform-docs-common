...
- One workspace per state file. Terraform migrate names the workspace following the `<DIRECTORY NAME>-<LOCAL WORKSPACE NAME>` pattern. Terraform migrate also creates workspace variables from the Terraform configuration's variables.
- One project to store all workspaces. Terraform migrate uses the directory path to the state file as the project name. For example, if your configuration is stored at `./frontend/networking/terraform.tfstate`, Terraform migrate names the project "frontend_networking". Because of this, your directory path must be between 3-40 characters and only include letters, numbers, inner spaces, hyphens, and underscores.
...
Refer to [Automate state file migration]() for instructions on how to use the command. 
...
| ------ | ----------- | ------- | -------- |
| `-config` | Specifies the path to an optional configuration file. Refer to  [Terraform migrate configuration file reference](PATH) for more information. | None | No |
...
|`--parallelism` | Specifies the number of threads Terraform migrate uses to scan the local directory and prepare the migration. Set this value to `1` to disable parallelism. | The number of logical CPUs in the system. | No |
