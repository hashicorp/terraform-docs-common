...
description: >-
  Terraform migrate configuration files let you control how to migrate your state to HCP Terraform or Terraform Enterprise.
---

# Terraform migrate configuration file reference

This topic describes the parameters for configuring Terraform migrate

## Configuration model

The following list outlines the parameters you can specify in a `tf-migrate` configuration file. Click on a property name to view additional details.
...
Specifies a list of project configurations that align with local directories. Terraform migrate creates one project in HCP Terraform or Terraform Enterprise per configuration. In each project, Terraform migrate creates one workspace per local workspace.
