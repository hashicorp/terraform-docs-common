...
The version is available in the Terraform Cloud UI. Consumers can begin using this provider version in configurations. You can repeat these steps starting from the [Create a provider platform](#create-a-provider-platform) step to add additional platform binaries for the release.
