...
To publish a new module:
  ...
  1. If presented, select either the **Tag** or **Branch** module publishing type.
...
- To change from tag-based to branch-based publishing, you must configure a **Module branch** and [create a new module version](#branch-based-publishing-workflow), as Terraform Could will not automatically create one.
...
You will be able to generate tests for your module once a version has been published. Tests can only be generated once per module and are intended to be reviewed by the module's authors before being checked into version control. If the module's configuration files exceed 128KB in total size, Terraform Cloud will not be able to generate tests for that module. You must have [permission to manage registry modules](/terraform/cloud-docs/users-teams-organizations/permissions#manage-private-registry) and the [module test generation setting enabled](/terraform/cloud-docs/users-teams-organizations/permissions#manage-private-registry) in order to generate tests.
