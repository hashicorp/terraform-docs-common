...
1. Verify that the module meets the [requirements for no-code provisioning](/cloud-docs/no-code-provisioning/module-design). 
...
To support the auto-apply workflow, ensure that downstream users can [automatically load provider credentials](/cloud-docs/no-code-provisioning/module-design#provider-credentials) into their new no-code workspaces, either through a global variable set with the credentials for the module's provider, or accessing outputs with credentials across workspaces. 
