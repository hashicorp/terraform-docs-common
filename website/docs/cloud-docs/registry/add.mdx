...
You can only add public registry modules hosted on GitHub to the HCP Terraform private registry or the Terraform Enterprise private registry that uses IPv4 addresses. This is because GitHub does not support IPv6. Refer to the [Terraform Enterprise deployment overview](/terraform/enterprise/deploy) for additional information. 
