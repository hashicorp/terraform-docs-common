...
tfc_only: true
description: >-
  Move to collaborative infrastructure-as-code workflows. Learn about Terraform Cloud's run environment, create workspaces, plan and create teams, assign permissions, and restrict non-Terraform access in this guide.
---
...
- Read the documentation on how to [perform and configure Terraform runs](/cloud-docs/run/remote-operations) in Terraform Cloud.
...
- Manage user accounts with [SAML single sign-on](/enterprise/saml/configuration). SAML support is exclusive to Terraform Enterprise, and lets users log into Terraform Cloud via your organization's existing identity provider. If your organization is at a scale where you use a SAML-compatible identity provider, we recommend this option.

  If your identity provider already has information about your colleagues' teams or groups, you can [manage team membership via your identity provider](/enterprise/saml/team-membership). Otherwise, you can add users to teams with the UI or with [the team membership API](/cloud-docs/api-docs/team-members).
...
Next, you can make additional improvements to your workflows and practices. Continue on to [Part 3.4: Advanced Improvements to Collaborative Infrastructure as Code](/cloud-docs/recommended-practices/part3.4).
