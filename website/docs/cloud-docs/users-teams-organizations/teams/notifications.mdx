...
* [**Manage teams**](/terraform/cloud-docs/users-teams-organizations/permissions#manage-teams) permissions for the team for which you want to configure notifications.
...
After creating a notification configuration, you can only edit certain aspects of that configuration:
...
To verify a notification’s configuration's success, the destination must respond with a `2xx` HTTP code. If verification fails, HCP Terraform displays an error message, and the configuration will remain disabled.
