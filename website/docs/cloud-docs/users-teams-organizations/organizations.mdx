...
When enabled, workspaces that are part of a shared repository will receive one status check that will aggregate all activity in response to an VCS event. This summary includes status checks that are scoped by organization and a configured VCS client connection.
