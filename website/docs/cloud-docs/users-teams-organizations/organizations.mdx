...
Terraform Cloud displays your current organization at the bottom of the navigation pane. To select an organization:
...
To join an organization, the organization's [owners][] must invite you and you must accept the emailed invitation. Refer to [Organization Settings: Users](#users) for details.
...
1. Provide an **Email address** where you can receive notifications about the organization.
1. Click **Create organization**.

Terraform Cloud shows the new organization in the navigation pane and prompts you to create a new workspace. You can also [invite other users](#users) to join the organization.

## Managing Settings

To view and manage an organization's settings, click **Settings** in the navigation pane.

The contents of the organization settings depend on your permissions within the organization. All users can review the organization's contact email, view the membership of any teams they belong to, and view the organization's authentication policy. [Organization owners][owners] can view and manage the entire list of organization settings. Refer to [Organization Permissions](/cloud-docs/users-teams-organizations/permissions#organization-permissions) for details.
...
All users in an organization can access the **Teams** page, which displays a list of [teams][] within the organiation. This page excludes [secret teams](/cloud-docs/users-teams-organizations/teams#team-visibility) where you are not a member. You can also view team membership and manage team API tokens.
...
1. Enter the user's email address and optionally add them to one or more teams. If the user accepts the invitation, Terraform Cloud automatically adds them to the specified teams.

You must manage all permissions in Terraform Cloud through teams. Users can join an organization without belonging to any teams, but they cannot use Teraform Cloud features until they belong to a team. Refer to [permissions](/cloud-docs/users-teams-organizations/permissions) for details.
...
Organization owners can set up a special [Organization API Token](/cloud-docs/users-teams-organizations/api-tokens) that has no association with a specific user or team.
...
Terraform Cloud paid features are available as a free trial. When a free trial expires, the organization displays a banner reading **TRIAL EXPIRED — Upgrade Required** in the navigation pane.
