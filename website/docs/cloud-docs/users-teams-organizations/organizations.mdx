...
Define configurable data retention policies for TFE organizations. The organization policy is the default policy applied to TFE workspaces, but it can be overridden on a per workspace basis. The workspace policy always overrides the organization policy.
