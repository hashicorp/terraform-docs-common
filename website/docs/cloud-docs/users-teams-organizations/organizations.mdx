...
- Continuous validation checks whether custom conditions in the workspace’s configuration continue to pass after Terraform has provisioned the infrastructure. Continuous validation requires Terraform version 1.3.0+.

You can enforce health assessments for all eligible workspaces or let each workspace opt in to health assessments through workspace settings. Refer to [Health](/cloud-docs/workspaces/health) in the workspaces documentation for more details.
