...
1. Navigate to the single sign-on page.
1. Edit step 2, "User Attributes & Claims."
   We recommend naming the claim "username", leaving the namespace blank, and sourcing `user.displayname` or `user.mailnickname` as a starting point. Note that Terraform Cloud usernames only allow lowercase letters, numbers, and dashes.
...
## Example For Team sync from AzureAd to TFC teams

To create group whose all members have access to some specific team of SSO with specific privileges :-
1. In the Azure portal, Create groups with any name and add members as per requirment .
1. Go to Single Sign-On tab and inside **Attributes & Claims** , Add a **GroupClaim** with name **MemberOf** .
1. Go to Terraform Cloud application and then select user & groups and then go insde required group , and copy the ObjectID form overview page . this we will use as SSO ID in next section .

## Configuration (Terraform Cloud)
1. In **Terraform Cloud portal** , create new team with required privileges , and paste the copied Object ID inside **SSO ID** tab inside team .
1. Now login with one of the member user of azure group in TFC via SSO , that user will have spceific provileges as mentioend in TFC organization-teams.

By above eaxample, the SSO user will be attached to secific team by defaut with desired permissions on TFC .
