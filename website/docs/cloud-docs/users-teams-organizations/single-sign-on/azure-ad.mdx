...
3. In the "Group Claims" side window, select the "Security Groups" radio button.  
   In the "Source Attribute" field, select either sAMAccountName to pass group names in the SAML or "Group ID" to pass thru the UUIDs for the group. See the related Note for more information on UUIDs.
   Check the "Customize the name of the group claim" box. Use "MemberOf" in the "Name (required)" field. Leave the namespace field blank.

-> **Note:** When you configure Azure AD to use Group Claims, it provides Group UUIDs instead of human readable names in its SAML assertions. We recommend [configuring SSO Team IDs](/cloud-docs/users-teams-organizations/single-sign-on#team-names-and-sso-team-ids) for your Terraform Cloud teams to match these Azure Group UUIDs.

If you plan to use SAML to set usernames in your Microsoft Azure AD application:
