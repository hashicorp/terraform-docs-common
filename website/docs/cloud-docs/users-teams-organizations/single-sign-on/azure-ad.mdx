...
1. Navigate to the single sign-on page.
1. Edit step 2, **User Attributes & Claims.**  
   We recommend naming the claim "Username", leaving the namespace blank, and sourcing something like `user.displayname` or `user.mailnickname`.  Note that HCP Terraform usernames only allow lowercase letters, numbers, and dashes.
