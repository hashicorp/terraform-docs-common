...
HCP Terraform expects the team names in the team membership SAML attribute to exactly match its own team names or its configured SSO Team IDs. This match is case sensitive and literal, HCP Terraform won't process the value passed by the IdP in any way, therefore values like full DN will not work.
