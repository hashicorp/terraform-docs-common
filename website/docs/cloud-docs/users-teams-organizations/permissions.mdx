...
Members of teams with "admin" permissions for a project have [General Workspace Permissions][#general-workspace-permissions] for every workspace in the project, as well as the ability to:
...
- Grant or revoke project permissions for visible teams. Project admins **cannot** view or manage access for teams that are are [Secret](/cloud-docs/users-teams-organizations/teams#team-visibility), unless those admins are also organization owners.
...
Separate from project and workspace permissions, you can grant teams permissions to manage certain resources or settings across an organization. To set these permissions for a team, go to your organization's **Settings**. Then click **Teams**, and select the team name from the list.
...
Does not grant members any explicit access to projects and workspaces. Permissions may still be granted to individual projects or workspaces through [Project Permissions](/cloud-docs/users-teams-organizations/permissions#project-permissions) and  [Workspace Permissions](/cloud-docs/users-teams-organizations/permissions#workspace-permissions).
