...
Allows members to publish and delete providers, modules, or both providers and modules using an organization's [claimed namespaces](/terraform/cloud-docs/registry/namespace) in the [public registry](https://registry.terraform.io/). These permissions are otherwise only available to organization owners.
