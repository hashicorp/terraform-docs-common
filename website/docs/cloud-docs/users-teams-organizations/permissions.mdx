...
There are three levels of team management permission that can be granted to teams: manage membership, manage teams, and manage organization access. They each provide unique abilities to manage teams and can require prerequisite permissions to be granted. For instance in order to grant manage teams permission manage membership must already be granted. In order to grant manage organization access permission both manage teams and manage membership permissions must be granted as well.
...
~> This permission is intended to allow owners of large organizations to delegate further team management control to another trusted team, and should be granted to only teams of trusted users. **Assign with caution:** Members with this permission will be able to update all organization access settings for any team visible to them.
...
This permission acts as a modifier to existing team management permissions. With it members can have access to secret teams up to the level permitted by other team management permissions. For instance the combination of access secret teams and manage teams permissions will allow members to create secret teams. Members with only this permission will be able to view all non-owners teams within the organization.
