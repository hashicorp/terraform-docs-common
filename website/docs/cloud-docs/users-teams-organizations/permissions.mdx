...
- **Other controls:**
  ...
  - **Lock/unlock workspace:** — Allows users to manually lock the workspace to temporarily prevent runs. When a workspace's execution mode is set to "local", this permission is required for performing local CLI runs using this workspace's state.
...
Some permissions imply other permissions; for example, the project access update permission also grants permission to read projects.
...
The following project permissions can be granted to teams on a per-project basis. They can be granted via either fixed permission sets or custom project permissions.
...
- **Project access:**
  ...
  - **Create Workspaces:** - Allow users to create workspaces in the project. Read access will be granted on all workspaces in the project.
  - **Delete Workspaces:** - Allows users to delete workspaces in the project.
    - Depending on the [organization's settings](/terraform/cloud-docs/users-teams-organizations/organizations#general), workspace admins may only be able to delete the workspace if it is not actively managing infrastructure. Refer to [Deleting a Workspace With Resources Under Management](/terraform/cloud-docs/workspaces/settings#deleting-a-workspace-with-resources-under-management) for details.
  - **Move Workspaces:** - Allows users to move workspaces out of the project. Note: the user must have Move Workspaces permission on both the source and destination project in order to successfully move a workspace from one project to another.
- **Team management:**
  ...
  - **Manaage:** — _Implies permission to read._ Allows users to set or remove project permissions for visible teams. Project admins cannot view or manage teams that are are [Secret](/terraform/cloud-docs/users-teams-organizations/teams#team-visibility), unless they are also organization owners.

See [General Workspace Permissions](#general-workspace-permissions) for the remainder of the custom permissions that can be granted for all workspaces in the project.

### Fixed Permission Sets

Fixed permission sets are bundles of specific permissions for a project, designed for common usage patterns.

#### Project Admin

Each project has an "admin" permissions level that grants permissions for both the project and the workspaces that belong to that project. Members of a team with admin permissions on a project are sometimes called "project admins" for that project.
...
You can use custom permissions to assign any of the permissions listed above under [General Project Permissions](#general-project-permissions).

Some permissions - such as the project access permission - are tiered: you can assign one permission per category, since higher permissions include all of the capabilities of the lower ones.
