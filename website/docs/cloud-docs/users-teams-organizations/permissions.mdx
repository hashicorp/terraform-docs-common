...
In order to modify the membership of a team, a user with Manage Membership permissions must have visibility into the team (i.e. the team must be ["Visible"](/cloud-docs/users-teams-organizations/teams#team-visibility), or the user must be on the team).
