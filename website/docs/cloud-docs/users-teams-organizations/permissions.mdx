...
There is a full [authorization matrix](/terraform/cloud-docs/architectural-details/security-model#authorization-model) available within the architectural security model documentation.
...
If the organization has enabled Single Sign On, these permissions will also be applied to the built-in team "sso" that is provisioned upon Single Sign On being successfully implemented. The only difference between the "sso" and "owners" teams is their method of login; after Single Sign On is enabled, all organization users including "sso" team members must login via the configured Single Sign On process, except "owners" team members, who may log in solely with Terraform Cloud native credentials.
...
There are three levels of team management permissions that can be granted to teams: manage membership, manage teams, and manage organization access. They each provide unique abilities to manage teams and can require prerequisite permissions to be granted. For instance, in order to grant the manage teams permission, manage membership must already be granted. In order to grant the manage organization access permission both manage teams and manage membership permissions must be granted as well.
...
This permission grants the ability to update a team names and SSO IDs but not its organization access settings. On its own this permission does not allow users to create, update, delete, or otherwise access secret teams.

This permission confers all of the permissions granted by the manage membership permission.

~> This permission is intended to allow owners of large organizations to delegate team management to another trusted team, and should be granted to only teams of trusted users. **Assign with caution:** Users with this permission are able to update or delete any visible team. Because this permission also confers the manage membership permission these users could also add themselves to any visible team.

#### Manage Organization Access

Allows members to update the organization access settings of teams.
...
~> This permission is intended to allow owners of large organizations to delegate further team management control to another trusted team, and should be granted only to teams of trusted users. **Assign with caution:** Members with this permission will be able to update all organization access settings for any team visible to them.

### Include Secret Teams

Allows members access to secret teams up to the level of permissions granted by their team permissions setting.

This permission acts as a modifier to existing team management permissions. With it members can have access to secret teams up to the level permitted by other team management permissions. For instance the combination of include secret teams and [manage teams permissions](/terraform/cloud-docs/users-teams-organizations/permissions#manage-teams) will allow members to create secret teams.
