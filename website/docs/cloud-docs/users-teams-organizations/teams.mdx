...
- [Team Tokens API](/terraform/cloud-docs/api-docs/team-tokens) to list, generate, and delete tokens
...
Each team can have an API token that is not associated with a specific user. You can manage this API token from the API tokens page, in the team token tab, where tokens can be created, regenerated and deleted. Refer to [Team API Tokens](/terraform/cloud-docs/users-teams-organizations/api-tokens#team-api-tokens) for details.
