...
To reach your user security settings page, sign in to [HCP Terraform](https://app.terraform.io/), click the user icon in the upper right corner, and choose **Account Settings** from the menu.
...
When you require two-factor authentication for all users and have users who [sign in with their HashiCorp Cloud Platform Account](/terraform/cloud-docs/users-teams-organizations/users#log-in-with-your-hashicorp-cloud-platform-account), the required configuration for each organization member depends on their linked HashiCorp Cloud identity:
