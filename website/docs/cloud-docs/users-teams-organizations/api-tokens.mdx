...
To manage the API token for a team, go to **Organization settings > API Tokens > Team Token tab** to create a new token or search for a team that you would like to manage the API token for.
