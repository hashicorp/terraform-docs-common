...
Review the organization name and contact email. Organization owners can change the organization's name, contact email, default execution mode, designated default project, or can delete the organization. When an organization owner updates the default execution mode, all workspaces that [inherit that value](/terraform/cloud-docs/workspaces/settings#execution-mode) will also inherit that execution mode.
