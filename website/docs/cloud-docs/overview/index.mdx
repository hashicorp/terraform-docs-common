...
Small teams can use most of Terraform Cloud's features for free, including remote Terraform execution, VCS integration, the private module registry, single-sign-on, policy enforcement, run tasks, and more.

Free organizations are limited to 500 managed resources. Refer to [What is a managed resource](/terraform/cloud-docs/overview/estimate-terraform-cloud-cost#what-is-a-managed-resource) for more details.
...
Terraform Cloud runs Terraform on disposable virtual machines in its own cloud infrastructure by default. You can leverage [Terraform Cloud Agents](/terraform/cloud-docs/agents) to run Terraform on your own isolated, private, or on-premises infrastructure. Remote Terraform execution is sometimes referred to as "remote operations."
...
Remote execution can be disabled on specific workspaces with the ["Execution Mode" setting][execution_mode]. The workspace will still host remote state, and Terraform CLI can use that state for local runs via the [Terraform Cloud CLI integration](/terraform/cli/cloud).
...
You can use projects to organize your workspaces into groups. Organizations with Terraform Cloud [Standard](https://www.hashicorp.com/products/terraform/pricing) Edition can assign teams permissions for specific projects. 
...
- More info: [Terraform State in Terraform Cloud](/terraform/cloud-docs/workspaces/state), [Run Triggers](/terraform/cloud-docs/workspaces/settings/run-triggers)
...
Terraform Cloud offers the best of both worlds, allowing you to run remote plans from your local command line. Configure the [Terraform Cloud CLI integration](/terraform/cli/cloud), and the `terraform plan` command will start a remote run in the configured Terraform Cloud workspace. The output of the run streams directly to your terminal, and you can also share a link to the remote run with your teammates.
