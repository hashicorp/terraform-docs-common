...
When you initially set up the workspace and add variables, or when the code in version control hasn't changed but you've modified some variables, you can manually start a run from the UI. Each workspace has an **+ New run** button in its header, which opens a **Start a new run** dialog for this purpose. When creating a new run, you can optionally provide a message and can choose between several modes, including Plan and apply (standard), [refresh-only](/terraform/cloud-docs/run/modes-and-options#refresh-only-mode), [plan-only (speculative)](/terraform/cloud-docs/run/modes-and-options#refresh-only-mode).

For runs including a plan phase, debugging mode can be enabled with the "Enable debugging mode" option. This is equivalent to setting the `TF_LOG` environment variable to a value of `TRACE` for this run. See [Debugging Terraform](/terraform/internals/debugging) for more information.

When choosing a normal plan and apply, you can include the [replace](/terraform/cloud-docs/run/modes-and-options#replacing-selected-resources) planning option.
...
Although any contributor to the repository can see the status indicators for pull request plans, only members of your Terraform Cloud organization with permission to read runs for the affected workspaces can click through and view the complete plan output. ([More about permissions.](/terraform/cloud-docs/users-teams-organizations/permissions))
