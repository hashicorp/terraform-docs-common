...
A workspace is linked to one branch of a VCS repository and ignores changes to other branches. Workspaces can also ignore some changes within their branch. If a Terraform working directory is configured, Terraform Cloud assumes that only some of the content in the repository is relevant to Terraform, and ignores changes outside of that content. You can also specify which files and directories within your repository trigger runs. Refer to [Automatic Run Triggering](/cloud-docs/workspaces/settings/vcs#automatic-run-triggering) for details.
