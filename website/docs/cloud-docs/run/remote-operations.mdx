...
[terraform_cloud_for_business_tier]: https://cloud.hashicorp.com/docs/hcp/admin/billing/flex-billing#terraform-cloud-business-tier
...
You can disable remote operations for any workspace by changing its [Execution Mode][execution_mode] to **Local**. This change causes the workspace to act only as a remote backend for Terraform state, with all execution occurring on your own workstations or continuous integration workers. Consequently, any [Apply][terraform_cloud_for_business_tier] that is run in Local Execution mode does not count towards any Flex meters or Service Entitlements.
