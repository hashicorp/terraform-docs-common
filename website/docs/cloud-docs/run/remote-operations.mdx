...
Terraform Cloud always performs Terraform runs in the context of a [workspace](/cloud-docs/run/remote-operations). The workspace serves the same role that a persistent working directory serves when running Terraform locally: it provides the configuration, state, and variables for the run.
