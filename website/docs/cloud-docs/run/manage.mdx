...
| Force Cancel Run    | A plan or apply was canceled, but Terraform Cloud was unable to end the run. Requires admin access to the workspace.  |
...
In rare cases, a cancelled run can fail to end, continuing to lock the workspace. You can forcefully cancel these runs, which immediately terminates the running Terraform process and unlocks the workspace.

Force-canceling requires admin access to the workspace because it can have dangerous side-effects, including loss of state and orphaned resources. Additionally, the **Force Cancel Run** button only appears after you click **Cancel Run** and Terraform Cloud has had time to terminate the run safely.
