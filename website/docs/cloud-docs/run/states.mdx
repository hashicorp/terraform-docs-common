...
- If the plan succeeded and requires changes:
  ...
  - If cost estimation is disabled and [Sentinel policies](/cloud-docs/policy-enforcement/sentinel) are enabled, the run proceeds automatically to the policy check stage.
