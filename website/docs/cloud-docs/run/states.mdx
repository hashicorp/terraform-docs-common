...
## 6. The Policy Check Stage

This stage only occurs if [Open Policy Agent (OPA) policies](/cloud-docs/policy-enforcement/opa) are enabled and runs after a successful `terraform plan` and before Cost Estimation. In this stage, Terraform Cloud checks whether the plan adheres to the policies in the OPA policy sets for the workspace.

_States in this stage:_
- **Policy Check:** Terraform Cloud is checking the plan against the OPA policy sets.
- **Policy Override:** The policy check finished, but a mandatory policy failed. The run pauses, and Terraform cannot perform an apply unless a user manually overrides the policy check failure. Refer to [Managing Policy Results](/cloud-docs/policy-enforcement/policy-results) for details.
...
- The run stops and enters the **Discarded** state when a user with [permission to apply runs](/cloud-docs/users-teams-organizations/permissions) discards the run.
