...
Most workflow partners create [run tasks](/terraform/cloud-docs/integrations/run-tasks) that let them integrate their services directly into the Terraform workflow. [Run tasks](/terraform/cloud-docs/integrations/run-tasks) let Terraform Cloud execute tasks in external systems at certain points in the Terraform Cloud run lifecycle. Specifically, Terraform Cloud users can configure run tasks to execute during the pre-plan, post-plan and the pre-apply run stages. Refer to the [Terraform Cloud Integrations](/terraform/docs/partnerships#terraform-cloud-integrations) or [Terraform Enterprise Integrations](/terraform/docs/partnerships#terraform-enterprise-integrations) documentation for more details.

Most workflow partners create run tasks that let them integrate their services directly into the Terraform workflow. Most workflow partners integrate with the Terraform workflow itself. Run tasks allow Terraform Cloud to execute tasks in external systems at specific points in the Terraform Cloud run lifecycle. This integration offers much more extensibility to Terraform Cloud customers, enabling them to integrate your services into the Terraform Cloud workflow. This feature allows users to add and execute these tasks during the pre-plan, post-plan and pre-apply stages. Eventually, we will open the third phase of this workflow to Terraform Cloud users, the post-apply stage. Refer to the [Terraform Cloud Integrations](/terraform/docs/partnerships#terraform-cloud-integrations) or [Terraform Enterprise Integrations](/terraform/docs/partnerships#terraform-enterprise-integrations) documentation for more details.
...
a) Get started with the Plugin Framework [repository](https://github.com/hashicorp/terraform-provider-scaffolding-framework)

b) Writing custom providers [guide](/terraform/plugin/framework)

c) Creating a Terraform Provider with the Plugin Framework: [video](https://www.youtube.com/watch?v=16qs7LJSyps)

d) An overview of the new Plugin Framework and improvements to the Terraform Provider ecosystem: [video](https://www.youtube.com/watch?v=pAeQC0fC3Xw)

e) Example provider for reference: [HTTP](https://github.com/hashicorp/terraform-provider-http)

f) Contributing to Terraform [guidelines](https://github.com/hashicorp/terraform/blob/master/.github/CONTRIBUTING.md)

g) HashiCorp Developer Discuss [forum](https://discuss.hashicorp.com/c/terraform-providers/tf-plugin-sdk/43)
...
Partners should build an integration using the [run tasks APIs in Terraform Cloud](/terraform/cloud-docs/api-docs/run-tasks/run-tasks). To learn more about how post-plan run tasks can enhance the Terraform Cloud workflow, refer to the following diagram and read our [run tasks announcement](https://www.hashicorp.com/blog/terraform-cloud-run-tasks-beta-now-available). For a more hands-on experience with run tasks follow the [HCP Packer run tasks learn guide here](/terraform/tutorials/cloud/setup-tfc-run-task).
