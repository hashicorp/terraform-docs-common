...
- **Infrastructure Partners** build Terraform providers (plugins) to empower customers/users to leverage Terraform to manage resources exposed by their platform APIs. These are accessible to users of all Terraform editions.
...
- **Security:** Partners that detect Terraform configuration errors that are misaligned with security and compliance policies.
...
Most workflow partners create run tasks that let them integrate their services directly into the Terraform workflow. Most workflow partners integrate with the Terraform workflow itself. Run tasks allow Terraform Cloud to execute tasks in external systems at specific points in the Terraform Cloud run lifecycle. This offers much more extensibility to Terraform Cloud customers, enabling them to integrate your services into the Terraform Cloud workflow. This feature allows users to add and execute these tasks during the post-plan and the new pre-plan & pre-apply stages. Eventually, we will open the third phase of this workflow to Terraform Cloud users, the post-apply stage. Reference the [Terraform Cloud Integrations](/docs/partnerships#terraform-cloud-integrations) or [Terraform Enterprise Integrations](/docs/partnerships#terraform-enterprise-integrations) documentation for more details.
...
Vendors who choose not to support their provider and prefer to make it a community-supported provider will no longer be listed as Partner.

-> Terraform modules published by Partner: We now have the ability to tag partner published modules in the Terraform Registry. If you have a modules that you’d like to receive the “Partner” tag, please reach out to us at technologypartners@hashicorp.com
...
Partners who successfully complete the Terraform Cloud integration checklist obtain a Terraform Cloud badge. This signifies HashiCorp has validated the integration under ideal conditions and the partner is a member of the HashiCorp Technology Partner Program.

- Note: As of September 2022, pre-plan & pre-apply run tasks are available only as a public beta feature, are subject to change, and not all customers will see this functionality in their Terraform Cloud organization. This feature is enabled by default for our business tier customers of Terraform Cloud. If you have a customer that is interested in run tasks and are not a current Terraform Cloud for Business customer, please ask them to [sign up for the public beta](https://docs.google.com/forms/d/e/1FAIpQLSf3JJIkU05bKWov2wXa9c-QV524WNaHuGIk7xjHnwl5ceGw2A/viewform).
...
-> **Note:** Pre-plan & pre-apply run tasks are only available as a public beta. [Updated September 1, 2022]

To better understand how pre-plan run task enhances the Terraform workflow, see diagram listed below. For additional API resources, [click here](https://developer.hashicorp.com/terraform/cloud-docs/api-docs/run-tasks/run-task-stages-and-results).
...
After successfully building a Run Tasks integration with Terraform Cloud, partners should contact their Alliance Manager to validate the integration against Terraform Enterprise. Many customers deploy Terraform Enterprise in a private cloud or air-gapped environment, so partners must ensure that their product can communicate bi-directionally with Terraform Enterprise inside these types of networks.

Upon successfully building Run Tasks integration with Terraform Cloud partners can reach out to their Alliance Manager to validate it against Terraform Enterprise. It’s common for customers to deploy Terraform Enterprise in a private cloud or in an air-gapped environment in which case partners need ensure that their product can communicate bi-directionally with TFE inside the customer’s network.