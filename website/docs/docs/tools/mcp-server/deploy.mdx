...
This topic describes how to deploy the Terraform model context protocol (MCP) server in a Docker container on your local workstation. The Terraform MCP server connects to a client so that your LLM can use data stored in the Terraform registry to help you write Terraform configuration. Refer to the [overview](/terraform/docs/tools/mcp-server) for additional information about MCP and the Terraform MCP server.
...
- Anthropic Claude Desktop. Refer to the [Anthroic Claude Desktop documentation](https://support.anthropic.com/en/articles/10065433-installing-claude-for-desktop) for installation instructions.
...
1. For VS Code, create an `mcp.json` file in the `.vscode` or `.cursor` directory of your local workspace and add the following configuration: 
   ...
   "servers": {
      "terraform-mcp-server": {
         ...
         "args": [
            ...
            "-i",
            "--rm",
            "quay.io/hashicorp-dev/terraform-mcp-server:0.0.1"
         ]
      }
   }
   }
...
1. For Claude Desktop MCP clients, create an `mcp.json` file and add the following configuration: 
...
1. Run the following command to pull the image and start the container:

   ```shell-session
   $ docker run quay.io/hashicorp-dev/terraform-mcp-server
   ```
